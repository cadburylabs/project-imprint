...
function listEditFieldOnKeyDown(evnt)
{
    ...
    else if(keyCode == 9 || keyCode == 13  || keyCode == 0 )
    {
...
function openNextEditField(listEditField, evnt)
{
	...
	var nextColumn =  keyCode== 9  || keyCode == 0  ? (bShiftKey ? listEditField.columnIndex-1 : listEditField.columnIndex+1) : listEditField.columnIndex;
	...
	if(spn == null)
    {
        nextColumn = keyCode == 9  || keyCode == 0  ? (bShiftKey && nextColumn<0 ? listEditFields[baseId].length-1 : 0) : nextColumn;
...
NLExtremeEditField.prototype.loadTextAreaContent = function NLExtremeEditField_loadTextAreaContent()
{
	...
	{
		var allowPermissionCheck = true;
        this.initExtraBackendParameter();
		this.isLoadingTextArea = true;
		this.input.value = "Loading Text...";
		var serverUrl = "/app/common/extreme/dynamiclisteditor.nl?loadtextarea=T&x_key="+this.span.getAttribute("rec_key");
		if(allowPermissionCheck) {
			serverUrl += "&rp="+this.fieldname;
			if(this.sBackendClass != null)
				serverUrl += "&back_class="+this.sBackendClass;
        }
        else {
			serverUrl += "&rp="+this.sTableAndColumnName;
        }
...
function showHideEmailIcon(span, newValue)
{
	...
	{
		var valid = newValue.length > 5;
		anchor.href = valid ? "mailto:"+newValue : '#';
		anchor.classList.toggle('uir-email-icon-anchor-empty', !valid);
	 }
}
...
NLExtremeEditField.prototype.close = function NLExtremeEditField_close()
{
        ...
        if (this.span._recordTooltip) {
            this.span._recordTooltip.enabled = true;
        }
	    this.span.removeAttribute('data-tooltip-enabled');

		var td = this.span.closest('td');
		if (td) {
			td.removeAttribute('data-edit')
		}
...
NLExtremeEditField.prototype.sendBackendRequest = function NLExtremeEditField_sendBackendRequest()
{
    ...
    if(runUpdate || this.applyUndo)
    {
        ...
        if(this.fieldtype == "select")
        {
	        ...
	        this.span.setAttribute("ntv_val", this.dbval);
        }
        else
        {
...
NLExtremeEditField.prototype.sendUpdateData = function NLExtremeEditField_sendUpdateData()
{
    ...
    if(bInBatchUpdate)
    {
        ...
        data["batch"] = "T";
        data["batchlist"] = this.baseId;
        data["batchcolumn"] = this.getColumnIndex();
    }
    else
    {
    ...
    data["_executexlist"] = "T";
    data["rp"] = this.fieldname;
    data["x_key"] = keys;
    data["newval"] = this.dbval;
    data["back_class"] = this.sBackendClass;

    
    if(this.hasSlave && this.slaveFieldValue != null)
    {
        data["slvrp"] = this.slaveFieldName;
        data["slvval"] = this.slaveFieldValue;
    ...
    if(this.sExtraBackendParam != null && this.sExtraBackendParam.length>0)
        data["x_extra"] = this.sExtraBackendParam;

    
    if(this.sExtraFieldValuePairs != null)
        data["x_fldvalpairs"] = this.sExtraFieldValuePairs;
    this.sExtraFieldValuePairs = null;

    NS.submitMutationAction("/app/common/extreme/dynamiclisteditor.nl", data, "extreme_list_commands");
};
...
NLExtremeEditField.prototype.setSpanAndDisplay = function NLExtremeEditField_setSpanAndDisplay(span)
{
    ...
    if(span.innerHTML == '&nbsp;'  || encodeURIComponent(span.innerHTML)=='%A0')
        this.currentValue = "";
...
NLExtremeEditField.prototype.displayInputFieldDiv = function NLExtremeEditField_displayInputFieldDiv()
{
	...
	if (this.span._recordTooltip) {
		this.span._recordTooltip.enabled = false;
	}
	this.span.setAttribute('data-tooltip-enabled', 'false');

	var td = this.span.closest('td');
	if (td) {
		td.setAttribute('data-edit', true);
	}
...
function getInputField(innerSpan, fieldType)
{
	var input = null;

	if(innerSpan != null)
	{
		input = innerSpan.firstChild;

		switch(fieldType) {
			case "textarea":
				input = innerSpan.querySelector("textarea");
				break;
		}
	}
...
NLExtremeEditField.prototype.buildEditFieldDiv = function NLExtremeEditField_buildEditFieldDiv()
{
    ...
    else
    {
		...
		this.input = getInputField(innerSpan, this.fieldtype);
...
function perform_delete_xlist_record(recordId, groupingId, isBatch, deletionReason, deletionReasonMemo)
{
	...
	NS.submitMutationAction(serverUrl, null, "extreme_list_commands");
...
function resetXlistOptionsImage(imageId, bUp)
{
    ...
    if(bUp)
        image.src = '/images/nav/listoptionsup.gif?v=2026.1';
    else
        image.src = '/images/nav/listoptionsdown.gif?v=2026.1';
}
...
{
	var serverUrl = "/app/common/extreme/dynamiclisteditor.nl?setquickadd=" + (bIson ? "T" : "F");
	NS.submitMutationAction(serverUrl, null, "list");
}
...
{
    var row = NS.jQuery("#scrollid" + sRecordNum).closest('.uir-list-row-tr');
    if (row.length)
    {
	    var scrollContainer = row.closest('.uir-list-body-wrapper');
		var header = scrollContainer.find('.uir-list-headerrow');
	    scrollContainer.scrollTop(row.position().top - header.height());
    }
}
...
{
    var currentVal = NLStringToNumber(nlef.input.value);
    ...
    if(nlef.fieldname == "rangelow")
    {
        ...
        if( idxMid > -1 && (currentVal > NLStringToNumber(nlef.getSiblingColumnValue(idxMid))) )
            nlef.setSiblingColumnValue(idxMid, currentVal, true);
        if( idxHigh > -1 && (currentVal > NLStringToNumber(nlef.getSiblingColumnValue(idxHigh))) )
    ...
    else if (nlef.fieldname == "projectedtotal")
    {
        ...
        var idxHigh = nlef.currentSiblingColumnIndex('rangehigh');
        if( idxLow > -1 && (currentVal < NLStringToNumber(nlef.getSiblingColumnValue(idxLow))) )
