    ...
    */
	...
	NS.UI.Button = (function () {
		...
		function setVisible(id, visible) {
			...
			if (NS.NextBridge) {
				NS.NextBridge.notify("nsUiButtonVisible", { name: id, value: visible});
			}
		}
		...
		function setLabel(id, text) {
			...
			if (button != null)
			{
				...
				if (NS.NextBridge) {
					NS.NextBridge.notify("nsUiButtonLabelChanged", { name: id, value: text});
				}
			}
		}
...
NS.UI.Util = NS.UI.Util || {
    ...
    isRedwood: false,
	isNetsuiteNext: false,
    isRefreshed: true,
	isClassicRefreshed: true,
	isClassicRedwood: false,
    isNextRedwood: false,
    isSuitePhone: false,
    getTop: function () {
	...
	alertPromise: function(msg){
		if (window.alert.isNextAlert) {
			return new Promise(function(resolve, reject){
				alert(msg, {onClosed: resolve});
			})
		} else {
			alert(msg);
...
function setFieldLabelRequired(fldName, required, fldForm)
{
	if (fldName)
	{
		...
		if (label)
		{
			...
			if (required && !asteriskLabel && !NS.UI.Util.isNetsuiteNext) {
...
function disableSelect(sel, val, win)
{
	if (!isBackend && (sel != null))
	{
		...
		if(NS.UI.Util.isNetsuiteNext){
			...
			if (wrapper) {
				wrapper.classList.toggle('uir-disabled', !!val);
			}
		}
	}
}
...
function nlExtOpenWindow(url, winname, width, height, fld, scrollbars, winTitle, listeners, triggerObj, attributes) {
	...
	content.onload = function () {
		...
		Object.keys(attrs).forEach(function (key){
			if(doc && doc.body){
				doc.body.setAttribute(key, attrs[key]);
			}
		})
	};
...
function collapsemachine(mname,sethidden,autoshow) {
	NS.UI.Personalization.Tab.collapse(mname, sethidden, NS.UI.Personalization.Tab.Type.SUBTAB);
	refreshSubTabMachine(mname, sethidden, autoshow);
}

function refreshSubTabMachine(mname,sethidden,autoshow) {
...
function simple_collapsemachine(mname, sethidden) {
	...
	NS.UI.Personalization.Machine.collapse(mname, sethidden);
}
