...
function getNow()
{
  ...
  var d = new Date();
  return new Date(d.getTime() + (-420 + d.getTimezoneOffset()) * 60000);
...
function getLongDate(date, useTruncatedNames)
{
	...
	var dateformat = "fmMonth DDfm, YYYY";
...
if ( false )
{
	...
	{
		NLDate_short_days[id] = NLDate_short_days[id].substr(1,1);
	}
}
...
function WriteCalendar( d, bIsInline)
{
        ...
        else
        {
            ...
            for (week=1,dofw=1,day=1;day<=lastday || dofw <= 7;)
            {
                ...
                else
                {
					...
					if(view=='day' && now.getDate() == day && now.getMonth()+1 == curmonth && now.getFullYear() == curyear){
						uirStyle += ' uir-popup-cal-cell--today';
                    }
    ...
    if ( !bIsInline && document.getElementById('calendarPortletId') != null && window.calendarPortletId != null)
    {
        ...
        if (iconDiv != null)
        {
            var tdContent = "<a class='portletIconHide' href='javascript:void(0);' title='" + label + "' onclick=\"javascript:hidePortlet(" + "-29" + "," + window.calendarPortletId.split("_")[1] + ",'" + window.calendarPortletId + "');\"></a>";
...
function writeDashboardPortletCalendar(d)
{
    ...
    var titleDateFormatInJs = "fmMonth fm, YYYY";
...
function NLDateField_handleKeyboardShortcut(element, evnt)
{
    var sDateFormat = window.dateformat;
    var dResult = null;

    
    var dateFormatWithMinus = sDateFormat ? (sDateFormat.indexOf("-") != -1) : false;
	var dataFormatWithLetter = sDateFormat ? (sDateFormat.toLowerCase().indexOf("mon") != -1) : false;

    var datestr = element.value;
    var parsedDate = parseDateOrTodayOnFail(datestr);

	var ev = getEvent(evnt);

	switch (ev.keyCode)
	{
        // Select next day / month
        case 39: // Arrow right
            if(ev.altKey) {
	            dResult = ev.shiftKey ? addmonths(parsedDate, 1) : adddays(parsedDate, 1);
            }
			break;
		// Select next day
		case 187: // "+" key on EN layout
		case 107: // "+" key on numeric keyboard
			dResult = adddays(parsedDate,1);
            break;

		// Select previous day / month
		case 37: // Arrow left
            if(ev.altKey) {
	            dResult = ev.shiftKey ?  addmonths(parsedDate, -1) : adddays(parsedDate, -1);
			}
			break;
		// Select previous day
		case 189: // "-" key on EN layout
		case 109: // "-" key on numeric keyboard
			if(!dateFormatWithMinus) {
				dResult = adddays(parsedDate, -1);
			}
            break;

		// Select today or tomorrow's date
        // 't' go to today, SHIFT + 't' go to tomorrow
        // ALT+t go to today and ALT+SHIFT+t go to tomorrow (the standard on other frameworks)
		case 84:
			if(ev.altKey || !dataFormatWithLetter) {
				dResult = ev.shiftKey ? adddays(new Date(), 1) : new Date();
            }
            break;

		// Select yesterday's date
        // 'y' go to yesterday
		// ALT+y go to yesterday (the standard on other frameworks)
		case 89:
            if(ev.altKey || !dataFormatWithLetter) {
	            dResult = adddays(new Date(), -1);
            }
            break;

		// ALT + HOME - Go to the first day of the selected month
		case 36:
			if(ev.altKey) {
				dResult = parsedDate;
				dResult.setDate(1);
		...
		// ALT + END - Go to the last day of the selected month
		case 35:
			if(ev.altKey) {
				dResult = parsedDate;
				dResult.setDate(NLDate_getLastDayOfMonth(parsedDate));
			}
			break;

		// ALT + PageUp - select next month
		case 33:
			if(ev.altKey) {
				dResult = addmonths(parsedDate, 1);
			}
			break;

		// ALT + PageDown - select previouse month
		case 34:
			if(ev.altKey) {
				dResult = addmonths(parsedDate, -1);
			}
		...
		// 'm' - Go to the last day of the current month
		case 77:
			if(ev.altKey || !dataFormatWithLetter)
			{
				dResult = new Date();
		...
		// 'l' - Go to the last day of the previous month
		case 76:
			if(ev.altKey || !dataFormatWithLetter)
			{
				dResult = new Date();
		...
		// 'p' go to end of period (if current view's got a period)
        case 80:
            if(!dataFormatWithLetter && window.pp_es)
            {
	...
	// if the key was not handled, check the event.key for sign '+' and '-'
    // these two key can have different keycode on different keyboard layouts, so it need to be checked by event.key property
	if(!dResult) {
		switch (ev.key) {
			// Select next day
			case "+":
    ...
    // if a shortcut key was input, we want any other change handlers on this field to be
    // fired correctly --  trigger validation on blur/change
    // @see NLField.java
    if( dResult )
    {
        var sNewValue = getdatestring(dResult);

        // explicitly set the 'changed' flag to true if the value has changed
        // wasn't firing the handler after the validation checker
        if( datestr != sNewValue )
        {
            ...
            // the browser does not call onchange() if the field is changed entirely via JS
            // as is the case with the accelerator keys;
            // explicitly call the onchange() function if one exists for this field
            if(element.onchange)
            {
                element.onchange();
            }
