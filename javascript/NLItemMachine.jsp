...
function isItemMachineCurrentLineValueEmpty(fieldName)
{
    return !itemMachCurrentLineValueNotEmpty(fieldName);
}
...
function setEncodedItemValueIfPresent(linenum, fieldname, value)
{
	if (itemHasEncodedField(fieldname))
	{
		setEncodedItemValue(linenum, fieldname, value);
	}
}

function hasPricesIncludeTaxField()
{
	return nlapiGetField('pricesincludetax') != null;
}

function setEncodedTipItemValue(linenum, fieldname, value)
{
	if (!hasPricesIncludeTaxField())
	{
		return;
	}

	var isTaxInclusive = nlapiGetFieldValue('pricesincludetax') === 'T';
	if (isTaxInclusive)
	{
		setEncodedItemValueIfPresent(linenum, fieldname, value);
	}
	else if (hasEncodedItemValue(linenum, fieldname))
	{
		setEncodedItemValueIfPresent(linenum, fieldname, '');
	}
}

function setItemMachValueIfPresent(fieldname, value)
{
	if (hasItemMachField(fieldname))
	{
		setItemMachValue(fieldname, value);
	}
}

function setTipItemMachValue(fieldname, value)
{
	if (!hasPricesIncludeTaxField())
	{
		return;
	...
	if (isTaxInclusive)
	{
		setItemMachValueIfPresent(fieldname, value);
	}
	else if (itemMachCurrentLineValueNotEmpty(fieldname))
	{
		setItemMachValueIfPresent(fieldname, '');
	}
}
...
function DoProduct(fldname)
{
	...
	{
		var rateStr = nlapiGetCurrentLineItemValue('item', rateField);
		var rate = parseFloat(rateStr);
		
		
		// if there is % in discount rate, find the correct basis line (skip Discount/Markup lines)
		// and calculate the discount amount from that basis.
		if (rateStr.indexOf('%') != -1 && nlapiGetCurrentLineItemIndex('item') > 1)
		{
			var basisLineIndex = nlapiGetCurrentLineItemIndex('item') - 1;
			for (var j = basisLineIndex; j > 0; j--)
			{
			...
			var basisField = taxInclusivePricingHelper.getEditableFieldName('amount');
			var basisLineAmount = nlapiGetLineItemValue('item', basisField, basisLineIndex);
			amtstr = format_currency(rate * 0.01 * basisLineAmount);
		}
		// Issue : 621741
		// When line location is changed with gross profit enabled, doProduct is triggered for gross estimate.
		// In case customer has manually entered the amount but left the rate empty, we do not want to override this value.
		// This branch only applies to non percentage discounts, percentage discounts must have rate filled in.
		else if (fldname == 'costestimaterate' && isNaN(rate) && !isNaN(amtstr)) {
			// Keep the amtstr as it was
		}
		else
		{
		...
		// For Discount/Markup, keep both current-line representations populated through the shared helpers.
		setItemMachValueIfPresent('amount', amtstr);
		setTipItemMachValue('grossamt', amtstr);
		setItemMachValueIfPresent('rate', rateStr);
		setTipItemMachValue('rateincludingtax', rateStr);
...
function safeDivide(numerator, denominator, defaultValue)
{
	...
	return isFiniteNumber(result) ? result : defaultValue;
}
...
function CheckDistrib()
{
  ...
  {
	var ddistrib = nlapiGetCurrentLineItemValue('item', 'ddistrib');
	var showedWarningItemIdsRaw = nlapiGetCurrentLineItemValue('item', 'showeddistributionwarning');
	var showedWarningItemIds = showedWarningItemIdsRaw === '' ? [] : showedWarningItemIdsRaw.split(',');
	var currentItemId = nlapiGetCurrentLineItemValue('item', 'item');
	var showedWarning = showedWarningItemIds.some(function (id) {
		return id === currentItemId;
	});
	if ((ddistrib == null || ddistrib == '') && !showedWarning)
	{
		...
		showedWarningItemIds.push(currentItemId);
		nlapiSetCurrentLineItemValue('item', 'showeddistributionwarning', showedWarningItemIds.join(','));
	}
  }
}
...
function SyncQtyRate()
{
	...
	if ((itemMachCurrentLineValueNotEmpty('orderdoc') || itemMachCurrentLineValueNotEmpty('srcline')) 
        && !hasItemMachField('isspecialorderline'))
		return;

    var overallQtyPricingBucketTotalQuantity = getItemMachFloat('quantity');

    if (itemMachCurrentLineValueNotEmpty('oqpbucket'))
    {
       
	    var overallQtyPricingBucket = getItemMachValue('oqpbucket');
	    var linecount = nlapiGetLineItemCount('item');
	    var currentLineNum = nlapiGetCurrentLineItemIndex('item');
	    var lineBeingEditedByUserUnitConversionRate = getItemMachValue('unitconversionrate');
        for (var j=1; j <= linecount; j++)
        {
            var lineQty=getEncodedItemFloat(j,'quantity');
			var currentLineUnitConversionRate = getEncodedItemValue(j, 'unitconversionrate');
            if (j != currentLineNum && getEncodedItemValue(j,'oqpbucket') == overallQtyPricingBucket && lineQty > 0){
				if (nlapiGetContext().getPreference("USE_UNIT_CONVERSION_RATE") === 'T' &&
                    currentLineUnitConversionRate != '' && lineBeingEditedByUserUnitConversionRate != ''
                ) {
					overallQtyPricingBucketTotalQuantity += lineQty * (currentLineUnitConversionRate / lineBeingEditedByUserUnitConversionRate);
                }
				else {
					overallQtyPricingBucketTotalQuantity += lineQty;
                }
            }
        }
    }
    ...
    if (itemMachCurrentLineValueNotEmpty('rateschedule') && !isNaN(overallQtyPricingBucketTotalQuantity) && !bCustomPriceLevel)
    {
        var schedstr = getItemMachValue('rateschedule');
        var marginal = 'T' == getItemMachValue('marginal');

		var rateField = taxInclusivePricingHelper.getEditableFieldName('rate');
		nlapiSetCurrentLineItemValue('item', rateField, format_rate(round_float(getQtyRate(schedstr, overallQtyPricingBucketTotalQuantity, marginal))), true);

	    
	}
}

//WARNING: this function is a mirror of recalcPoRateBasedOnLineQuantity, this DOES require the line commit (Issue 643030)
...
//WARNING: this function is a mirror of SyncQtyPurchRate, this DOES NOT require the line commit (Issue 643030)
function recalcPoRateBasedOnLineQuantity(lineNumber)
{
    var schedstr = getEncodedValue('item', lineNumber, 'poratesched');
    var lineQuantity = getEncodedValue('item', lineNumber, 'quantity');
	var qty = parseFloat(lineQuantity);
	// Make sure all the fields are there. Otherwise, bail out.
	if (schedstr == null || schedstr.length == 0 || lineQuantity == null || isNaN(qty))
		return;
    var marginal = 'T' === getEncodedValue('item', lineNumber, 'pomarginal');
    var overallqtydisc = getEncodedValue('item', lineNumber, 'pooverallqtydisc');

    if (overallqtydisc.length > 0)
    {
        var linecount = nlapiGetLineItemCount('item');
        var vendor = parseInt(getEncodedValue('item', lineNumber, 'povendor'));
        var qtygroup = parseInt(getEncodedValue('item', lineNumber, 'poqtygroup'));
        var createpo = getEncodedValue('item', lineNumber, 'createpo');
        if (vendor > 0)
        {
            for (var j = 1; j <= linecount; j++)
                if (j != lineNumber && getEncodedValue('item', j, 'pooverallqtydisc') === overallqtydisc && parseInt(getEncodedValue('item', j, 'poqtygroup')) == qtygroup && parseInt(getEncodedValue('item', j, 'povendor')) == vendor && ((getEncodedValue('item', j, 'createpo').length == 0 && createpo.length == 0) || getEncodedValue('item', j, 'createpo') == createpo) && parseFloat(getEncodedValue('item', j, 'quantity')) > 0)
                {
                    qty += parseFloat(getEncodedValue('item', j, 'quantity'));
                }
        }
    }
	if (!isNaN(qty))
	{
		setEncodedValue('item', lineNumber, 'porate', format_currency(round_float(getQtyRate(schedstr, qty, marginal)), true));
	}
}
...
function CheckAmount()
{
  ...
  else if (itemtype == 'GiftCert' && parseFloat(nlapiGetCurrentLineItemValue('item','amount')) < 0)
    alert('The gift certificate amount cannot be negative.');
...
function shouldSkipComponentYieldCheck() {
	return getTransactionType() !== 'workord' || isItemMachineCurrentLineValueEmpty('componentyield') || isItemMachineCurrentLineValueEmpty('bomquantity');
}

function checkComponentYieldQuantityChange()
{
	if(shouldSkipComponentYieldCheck()) {
		return true;
	}

	// the string representation may contain the special % character, which may cause the parseFloat function to fail and return NaN
	var componentYieldString = nlapiGetCurrentLineItemValue('item', 'componentyield');
	var componentYield = parseFloat(componentYieldString.substring(0, componentYieldString.length - 1));

	if (componentYield <= 0 || componentYield > 100) {
		alert("The component yield must be greater than 0% and at most 100%.");
		return false;
	}
	return true;
}
...
function FillPctQty(sync)
{
	...
	for (var i=1; i <= linecount; i++)
	{
		...
		if (!isNaN(ordered))
		{
			...
			setEncodedValue('item', i, 'quantity', (oldqty < 0) ? -newqty : newqty);
			if (sync)
			{
			...
			setEncodedValue('item',i,'costestimate',format_currency(getEncodedValue('item',i,'costestimaterate')*getEncodedValue('item',i,'quantity')));
		}
	}
	Item_Machine_Recalc();
	if (hasLegacyMachine())
...
function Item_Machine_Recalc(onload) {
    ...
    var oqpHandler = new function()
    {
        ...
        this.init = function (){
            ...
            else if (isUseUnitConversionRatePreferenceEnabled)
            {
                loadBucketTotalsAndSetNeedsRecalc(1);
            }
        };
        ...
        function loadBucketTotalsAndSetNeedsRecalc(linenum){
            ...
            for (var j=1; j <= linecount; j++)
            {
                var quantityToAdd = getEncodedItemFloat(j,'quantity');

                if ( !lineImpactsOQP(j) || quantityToAdd == 0)
                {
                    continue;
                }
                ...
                if (isUseUnitConversionRatePreferenceEnabled && currentLineUnitConversionRate !== '')
                {
                    quantityToAdd *= currentLineUnitConversionRate
                }

                if (isNaN(bucketTotals[bucketKey]))
                {
                    bucketTotals[bucketKey] = quantityToAdd;
                }
                else
                {
                    bucketTotals[bucketKey] += quantityToAdd;
        ...
        function setLineRateAndAmount(linenum, overallQuantity)
        {
            var rateIsMarginal = 'T' == getEncodedItemValue(linenum, 'marginal');
            var rate = getQtyRate(getEncodedItemValue(linenum, 'rateschedule'), overallQuantity, rateIsMarginal);
        ...
        this.handleLineRecalc = function (linenum) {
            if (lineImpactsOQP(linenum)
                && !lineIsFromTransform(linenum)
                && !isManualRate(linenum)
                && getTransactionType() !== 'rtnauth')
            {
                var overallQuantity = getBucketQty(linenum);
	            
                if (isUseUnitConversionRatePreferenceEnabled)
                {
                    var currentLineUnitConversionRate = getEncodedItemValue(linenum, 'unitconversionrate');
                    if (currentLineUnitConversionRate !== "") {
                        overallQuantity = overallQuantity / currentLineUnitConversionRate
                    }
                    setLineRateAndAmount(linenum, overallQuantity);
                } else if (!skipOQPRecalcForCurrentLine && bucketNeedsRecalc(linenum))
                {
                    setLineRateAndAmount(linenum, overallQuantity);
                }
            }
	...
	for (var i=1; i <= linecount; i++)
	{
		...
		var isDiscountItem = getEncodedValue('item', i, 'itemtype') == 'Discount';
		var isMarkupItem = getEncodedValue('item', i, 'itemtype') == 'Markup';

        if (isDiscountItem || isMarkupItem){
            if (i==1) {
            ...
            var basisField = (nlapiGetFieldValue('pricesincludetax') === 'T') ? 'grossamt' : 'amount';
            var basis = parseFloat(getEncodedValue('item', basisline, basisField));
            var asbasis=parseFloat(getEncodedValue('item', basisline, 'altsalesamt'));

            if (isFiniteNumber(basis)){
                var manuallyEnteredTipAmount = (nlapiGetFieldValue('pricesincludetax') === 'T') && hasEncodedItemValue(i, 'grossamt') && !hasEncodedItemValue(i, 'rate') && !hasEncodedItemValue(i, 'rateincludingtax');
                if (manuallyEnteredTipAmount)
                {
                    var manualGrossAmount = getEncodedValue('item', i, 'grossamt');
                    setEncodedItemValueIfPresent(i, 'amount', manualGrossAmount);
                }

                var rawRateStr = getEncodedValue('item', i, 'rate');
                if (!rawRateStr || rawRateStr.length === 0)
                {
                    rawRateStr = getEncodedValue('item', i, 'rateincludingtax');
                }
                if ((!rawRateStr || rawRateStr.length === 0) && nlapiGetContext().getExecutionContext() == 'userinterface')
                {
                    rawRateStr = nlapiGetLineItemValue('item', 'rate', i);
                    if (!rawRateStr || rawRateStr.length === 0)
                    {
                        rawRateStr = nlapiGetLineItemValue('item', 'rateincludingtax', i);
                    }
                }

                var rate = parseFloat(rawRateStr);
                if (isFiniteNumber(rate))
                {
                    var isRatePct = rawRateStr.indexOf('%') != -1;
                    var computedDiscountAmount = (isRatePct ? safeDivide(rate * basis, 100, null) : rate);
                    if (computedDiscountAmount !== null)
                    {
                        var amtstr = format_currency(computedDiscountAmount);

						setEncodedItemValueIfPresent(i, 'amount', amtstr);
						setEncodedTipItemValue(i, 'grossamt', amtstr);
						setEncodedItemValueIfPresent(i, 'rate', rawRateStr);
						setEncodedTipItemValue(i, 'rateincludingtax', rawRateStr);
					}

					callRecordScript({eventName: 'onSubtotalAmountSet', arguments:{sublistId: 'item', lineNum: i, lineCount: linecount, amount: amtstr, itemType: 'DiscountOrMarkup'}, isCustomMethodCall: true});
}

				if (hasEncodedField('item', 'taxableamt')){
					var taxableItemAmount = parseFloat(getEncodedValue('item', i, 'amount'));
					if(!isNaN(taxableItemAmount))
					{
						if (isMarkupItem)
						{
							setEncodedValue('item', i, 'taxableamt', getEncodedValue('item', i, 'istaxable') == 'T' ? taxableItemAmount : '');
						}
						else if (isDiscountItem)
						{
							...
							if (isNaN(taxbasis) && getEncodedValue('item', basisline, 'istaxable') == 'T')
								taxbasis = basis;
							var safeTaxableAmount = safeDivide(taxableItemAmount * taxbasis, basis, 0);
							setEncodedValue('item', i, 'taxableamt', safeTaxableAmount);
						}
					}
            ...
            if (!isNaN(asbasis))
            {
                ...
                var adjustedAltSalesAmount = safeDivide(rate * asbasis, discBasis, null);
                var asamtstr = (adjustedAltSalesAmount === null ? '' : format_currency(adjustedAltSalesAmount));
        ...
        else if (getEncodedValue('item',i,'itemtype') == 'Subtotal')
        {
			...
			callRecordScript({eventName: 'onSubtotalAmountSet', arguments:{sublistId: 'item', lineNum: i, lineCount: linecount, amount: amtstr, itemType: 'Subtotal'}, isCustomMethodCall: true});
        }
        ...
        else if (getEncodedValue('item',i,'itemtype') == 'Group' && getEncodedValue('item',i,'groupsetup') == 'T')
        {
            ...
            var hasGroupEndItem = false;
            for (var j = i + 1; j <= linecount; j++)
            {
                if (getEncodedValue('item', j, 'itemtype') === 'Group')
                    break;
                if (getEncodedValue('item', j, 'itemtype') === 'EndGroup') {
                    hasGroupEndItem = true;
                    break;
                }
            }
        ...
        if (isVat)  {
            ...
            if (parseFloat(getEncodedValue('item',i,'refamt'))!=parseFloat(getEncodedValue('item',i,'amount'))) {
                ...
                if (!isNaN(vatrate)){
					if (hasEncodedField('item', 'tax1amt'))
					{
						/*The 'grossamt' field condition was included because of Issue 684362
                          Outsourced PO does not contain 'grossamt' field and therefore the body
                          of syncVatLine2 funtion remains empty and cannot calculate tax1amt*/
						if (hasEncodedField('item', 'grossamt') && typeof syncVatLine2 === 'function')
                        	syncVatLine2('amount', 'item', i);
						else
...
function Item_Machine_ValidateLine()
{
  ...
  if(!checkComponentYieldQuantityChange()) {
	  return false;
  }
  ...
  if (hasEncodedField('item','linked'))
  {
    ...
    if (getItemMachValue('linked') == 'T')
    {
	  ...
	  if(warning != null)
	  {
		  var item_hasfulfillment = 'F';
		  if (getItemMachValue('itempicked') == 'T' || getItemMachValue('itempacked') == 'T' || getItemMachValue('linkedshiprcpt') == 'T')
		  {
			  item_hasfulfillment = 'T'
		  }
          if (typeDoesNotAllowItemChangeForAlreadyLinked(type, item_hasfulfillment) && selectedItemHasChanged())
          {
              
	          switch (warning)
	          {
		          case 'received':
			          alert("You cannot change the selected item because it has already been received.");
			          break;
		          case 'picked':
			          alert("You cannot change the selected item because it has already been picked.");
			          break;
		          case 'packed':
			          alert("You cannot change the selected item because it has already been packed.");
			          break;
		          case 'fulfilled':
			          alert("You cannot change the selected item because it has already been fulfilled.");
			          break;
		          default:
	      ...
	      { 
	        sPrompt="You are editing a Return Authorization that has already been received. Changing an item that has already been received can affect reimbursement.\n\nClick OK to save your changes on the Return Authorization.\nClick Cancel to go back to the Return Authorization.";
	      }
		  ...
		  else
		  {
			  ...
			  {
				  case 'returned':
					  sPrompt = "Items on this line have been returned. Are you sure you want to modify this item?";
					  break;
				  case 'fulfilled':
					  sPrompt = "Items on this line have been fulfilled. Are you sure you want to modify this item?";
					  break;
				  case 'received':
					  sPrompt = "Items on this line have been received. Are you sure you want to modify this item?";
					  break;
				  case 'included in build':
					  sPrompt = "Items on this line have been included in the build. Are you sure you want to modify this item?";
					  break;
				  case 'requested':
					  sPrompt = "Items on this line have been requested. Are you sure you want to modify this item?";
					  break;
				  case 'packed':
					  sPrompt = "Items on this line have been packed. Are you sure you want to modify this item?";
					  break;
				  case 'picked':
					  sPrompt = "Items on this line have been picked. Are you sure you want to modify this item?";
					  break;
				  case 'drop shipped or special ordered':
					  sPrompt = "Items on this line have been drop shipped or special ordered. Are you sure you want to modify this item?";
					  break;
				  case 'revenue committed':
					  sPrompt = "Items on this line have had their revenue committed. Are you sure you want to modify this item?";
					  break;
				  case 'billed':
					  sPrompt = "Items on this line have been billed. Are you sure you want to modify this item?";
					  break;
				  case 'received and billed':
					  sPrompt = "Items on this line have been received and billed. Are you sure you want to modify this item?";
					  break;
				  case 'fulfilled and billed':
					  sPrompt = "Items on this line have been fulfilled and billed. Are you sure you want to modify this item?";
					  break;
				  default:
					  alert("An unexpected error occurred.");
					  return false;
  ...
  checkItemLineShippingFields();
	...
	if (hasItemMachField(amountField) && getTransactionType() != 'purchreq' && (getItemMachValue(amountField) == undefined || getItemMachValue(amountField).length == 0) && getItemType() != 'Description' && getItemType() != 'Group' && getItemType() != 'EndGroup' && getItemType() != 'Subtotal' && getItemType() != 'SubscriPlan')
	{
		...
		if ((!hasItemMachField(mandatoryAmountField)) || (getItemMachValue(mandatoryAmountField) != 'F')) {
			alert(format_message('Please enter a value for {1}', nlapiGetLineItemLabel('item', amountField)));
			return false;
		}
	}
  ...
  {
	  var giftCertFieldsMandatory = nlapiGetContext().getPreference('GIFT_CERTIFICATE_FIELDS_MANDATORY') == 'T';

    if (giftCertFieldsMandatory && hasItemMachField('gcfields'))
    {
...
// if serial numbers field is invalid, line is invalid
  if ( hasItemMachField('serialnumbersvalid'))
  {
    ...
    var isWorkOrderOrTransferOrder = type == 'workord' || type == 'trnfrord';
	if (( !isWorkOrderOrTransferOrder && !CheckMinCount()) || !CheckCount())
		return false;
	...
	if (type == salesOrderTranType)
	{
		...
		if (isUserAction)
		{
			...
			var isItemPurchasable = nlapiServerCall('/app/site/backend/validatecatalogitempurchasability.nl', 'isItemPurchasable', [itemId]);
	...
	if (nlapiGetContext().getFeature('PREPAYWITHDRAWDOWN') && (type == 'custinvc' || type == 'custcred')
		&& (getItemMachValue('chargetype') == '-25')
        && (getItemMachValue('subscriptionprepaylinkedusagecharge')) && hasLegacyMachine())
	{
		for (var i = 0; i < item_machine.form_elems.length; i++)
		{
			var currentElementName = item_machine.form_elems[i];
			if (fieldOnItemMachineHasChanges(currentElementName))
			{
				var previousValue = nlapiGetLineItemValue('item', currentElementName, nlapiGetCurrentLineItemIndex('item'));
				nlapiSetCurrentLineItemValue('item',currentElementName, previousValue, true, true);
			}
		}
		alert('You cannot edit this child transaction line that is associated with a parent transaction line. You must edit the parent transaction line.');
	}

	updatePrepayLineFields();

	if (!validateWorkOrderClosedLine())
	{
		return false;
    }
...
function validateWorkOrderClosedLine()
{
	if (getTransactionType() === 'workord' && hasItemMachField('isclosed'))
	{
		var originalWorkOrderStatus = nlapiGetFieldValue('origstatus');
		var workOrderStatus = nlapiGetFieldValue('orderstatus');
		var isClosed = getItemMachValue('isclosed');

		var allowedTransitions = [
			{ from: 'D', to: 'G' }, // In Process -> Closed
			{ from: 'G', to: 'D' }, // Built -> In Process
			{ from: 'A', to: 'H' }, // Planned -> Closed
			{ from: 'B', to: 'H' }  // Released -> Closed
		];

        // Do not validate if the current transition is allowed
		for (var i = 0; i < allowedTransitions.length; i++) {
			var transition = allowedTransitions[i];
			if (originalWorkOrderStatus === transition.from && workOrderStatus === transition.to) {
				return true;
			}
		}

		if (isClosed === 'T' && workOrderStatus !== 'G' && workOrderStatus !== 'H') {
			alert('Work Order line cannot be closed.');
			return false;
		}

		if (isClosed === 'F' && (workOrderStatus === 'G' || workOrderStatus === 'H')) {
			alert('Work Order line cannot be opened.');
			return false;
		}
	}
...
function updatePrepayLineFields()
{
    var isApply = nlapiGetContext().getFeature('PREPAYWITHDRAWDOWN')
        && (getTransactionType() == 'opprtnty' || getTransactionType() == 'salesord' || getTransactionType() == 'estimate')
        && (getItemMachValue('subscriptionlinetype') == '5');

    if (isApply) {
        var prepayAmount = getItemMachValue('subscriptionprepaylineamount');
        setItemMachValue('quantity', '1');
	    setItemMachValue('rate', prepayAmount);
	    setItemMachValue('amount', prepayAmount);
    }
}
...
function Item_Machine_postDeleteLine(line, origLineData)
{
	...
	if (typeof changeNexusDueToMossRelatedChangeIfNeeded  === 'function')
	{
		changeNexusDueToMossRelatedChangeIfNeeded(true);
	}

	//When a usage line is deleted, we check to see if it has any associated prepayment line
    if (nlapiGetContext().getFeature('PREPAYWITHDRAWDOWN') && (getTransactionType() == 'custinvc' || getTransactionType() == 'custcred'))
		deleteChildPrepaymentLines(origLineData);

	return true;
}

var isParentUsageDelete = false;

function deleteChildPrepaymentLines(origLineData)
{
	var lineChargeType = getOrigLineData(origLineData, 'chargetype');

	if (lineChargeType == '-16' || lineChargeType == '-26')
	{
		var parentUsageCharge = getOrigLineData(origLineData, 'charge');
		var childPrepaymentLine = nlapiFindLineItemValue('item', 'subscriptionprepaylinkedusagecharge', parentUsageCharge);

		//We expect a 1-1 relationship between parent usage and child prepayment lines
		while (childPrepaymentLine != -1)
        {
	        isParentUsageDelete = true; //so that the delete prevention alert isn't called
			nlapiRemoveLineItem('item', childPrepaymentLine);
			childPrepaymentLine = nlapiFindLineItemValue('item', 'subscriptionprepaylinkedusagecharge', parentUsageCharge);
		}
		isParentUsageDelete = false;
	}
...
function enableDisableConsignedField()
{
	
}

function updateLineLevelConsignedAfterVendorChange()
{
	if (!isValEmpty(nlapiGetFieldValue('representssubsidiary')))
	{
		for (var i = 1; i <= nlapiGetLineItemCount('item'); i++)
		{
			nlapiSetLineItemValue('item', 'consigned', i, 'F');
			nlapiSetLineItemDisabled('item', 'consigned', true, i);
		}
	}
}

function enableDisableMatchBillToReceiptField()
{
	
}

function validateConsignedField()
{
	
}

function defaultLineLevelConsignedValueToHeaderValue()
{
...
function Item_Machine_ValidateDelete()
{
  ...
  if (itemHasEncodedField('linked'))
  {
    if (getItemMachValue('linked') == 'T')
    {
	  ...
	  else
	  {
          ...
          {
			  case 'received':
				  alert("Items on this line have been received. To delete this line, you must first delete the corresponding line(s) in the associated transaction(s).");
				  break;
			  case 'fulfilled':
				  alert("Items on this line have been fulfilled. To delete this line, you must first delete the corresponding line(s) in the associated transaction(s).");
				  break;
			  default:
				  alert("An unexpected error occurred.");
		  }
  ...
  // Start of the group shouldn't be deleted before the corresponding lines are removed for fulfilled sales order
  if (itemType === 'Group' && word === 'fulfilled')
  {
	  var isItemGroupFulfilled = false;

	  for (var j= i+1; j < parseInt(nlapiGetLineItemCount('item') + 1); j++) {
		  if (getEncodedValue('item', j, 'itemtype') === 'EndGroup') {
			  break;
		  }
		  if (nlapiGetLineItemValue('item', 'linked', j) === 'T') {
			  isItemGroupFulfilled = true;
			  break;
		  }
	  }

	  if (isItemGroupFulfilled) {
		  alert("Items on this line have been fulfilled. To delete this line, you must first delete the corresponding line(s) in the associated transaction(s).");
		  return false;
	  }
  }
  ...
  nlapiSetFieldValue('doshippingrecalc', 'T');
	...
	if (nlapiGetContext().getFeature('PREPAYWITHDRAWDOWN') && (type == 'custinvc' || type == 'custcred')
        && (getItemMachValue('chargetype') == '-25')
        && (getItemMachValue('subscriptionprepaylinkedusagecharge')) && !isParentUsageDelete)
	{
		alert('You cannot delete this child transaction line that is associated with a parent transaction line. You must delete the parent transaction line.');
		return false;
    }
...
function calculateRateAndAmountBeforeCopy(line)
{
	var contractId = nlapiGetLineItemValue('item', 'purchasecontract', line);
	var contractDetails = nlapiGetLineItemValue('item', 'contractdetails', line);
	if (!isValEmpty(contractId) && !isValEmpty(contractDetails))
		calculateContractRateForAllLines(contractId, contractDetails, false, false);
}
...
function Item_Machine_synclinefields(line)
{
	...
	var isIncorrectRateTierFixCanaryOn = false;

	if (isIncorrectRateTierFixCanaryOn)
	{
		if (getTransactionType() == 'purchord')
		{
			if (this.ischanged == false && this.isdeleting == false && this.isinline == true && this.isinserting == false)
			{
				calculateRateAndAmountBeforeCopy(line);
            }
		}
	}
	...
	disablePrepaymentLinkedFields();
	enableDisableConsignedField();

	if (hasItemMachField('customer'))
		updateBillableEnableFlag();
}

function disablePrepaymentLinkedFields()
{
	var isApply = nlapiGetContext().getFeature('PREPAYWITHDRAWDOWN') && (getTransactionType() == 'custinvc' || getTransactionType() == 'custcred')
		&& (getItemMachValue('chargetype') == '-25')
        && (getItemMachValue('subscriptionprepaylinkedusagecharge'));

	if (isApply && hasLegacyMachine())
	{
        for (var i = 0; i < item_machine.form_elems.length; i++) {
            var currentElementName = item_machine.form_elems[i];
            setItemMachDisabled(currentElementName, true);
        }

		NS.UI.Button.setVisible('item_remove', false);
		NS.UI.Button.setVisible('item_copy', false);
		NS.UI.Button.setVisible('item_insert', false);
	...
	if(hasLegacyMachine())
    {
	    NS.UI.Button.setVisible('item_remove', true);
	    NS.UI.Button.setVisible('item_copy', true);
	    NS.UI.Button.setVisible('item_insert', true);
    }
}
...
{
	if (canIgnoreLine()) return;

	if (getTransactionType() == 'workord' && nlapiGetCurrentLineItemValue('item', 'itemtype') == '') {
		if (nlapiGetCurrentLineItemValue('item', 'requesteddate') != '') {
			nlapiSetCurrentLineItemValue('item', 'requesteddate', '', true);
		}
		if (nlapiGetCurrentLineItemValue('item', 'expectedshipdate') != '') {
...
function setAllocationStrategy()
{
	...
	if(hasAllocationStrategy) {
		...
		else if ('salesorder' == nlapiGetRecordType() && nlapiGetCurrentLineItemValue('item', 'defaultitemallocationstrategy') != '') {
			nlapiSetCurrentLineItemValue('item', 'orderallocationstrategy', nlapiGetCurrentLineItemValue('item', 'defaultitemallocationstrategy'));
		}
		else {
...
{
	if(nlapiGetCurrentLineItemValue('item','commitmentfirm') == 'T' && nlapiGetCurrentLineItemValue('item','oldcommitmentfirm') == 'T' && nlapiGetCurrentLineItemValue('item','orderallocationstrategy') != '') {
...
function checkFulfilledQuantity()
{
	if(nlapiGetFieldValue('saleschannel') == null || nlapiGetFieldValue('saleschannel') =='')
		return;
	var i = parseInt(nlapiGetCurrentLineItemIndex('item'));
	var newqty = parseFloat(getItemMachValue('quantity'));
	var oldqty = parseFloat(getEncodedValue('item',i,'quantity'));
	var baseqty = parseFloat(nlapiGetCurrentLineItemValue('item','quantitypickpackship'));
	if(newqty < baseqty)
    {
	    nlapiSetCurrentLineItemValue('item', 'quantity', oldqty);
	    alert('SO quantity cannot be less than the sum of all quantities picked, packed, or shipped for channel sales order.');
    }
}
...
function handleIsClosedChanged() {
	var isClosed = nlapiGetCurrentLineItemValue('item', 'isclosed') === 'T';
	var woKey = nlapiGetCurrentLineItemValue('item', 'createdoutsourcedwokey');
	if (isClosed && !woKey) {
		nlapiSetCurrentLineItemValue('item', 'createoutsourcedwo', 'F');
		nlapiSetLineItemDisabled('item', 'createoutsourcedwo', true);
	}
	else if (!isClosed && !woKey) {
		nlapiSetLineItemDisabled('item', 'createoutsourcedwo', false);
	}
}
...
function unsetBomRevisionComponent()
{
	if (hasItemMachField('bomrevisioncomponent') &&
        (nlapiGetCurrentLineItemValue('item', 'item') != nlapiGetCurrentLineItemValue('item', 'olditemid'))) {
		setItemMachValue('bomrevisioncomponent', null);
    }
}
...
function syncComponentYieldQuantity()
{
    ...
    {
		nlapiSetCurrentLineItemValue('item', 'quantity', getComponentYieldQuantity(), true);
	}
}
...
function popupScheduleDiv(schedstr, fld)
{
	...
	var fldPosition = NS.jQuery(fld).offset();
    scheduleDiv.style.left = fldPosition.left + 'px'; //findPosX(fld)
    scheduleDiv.style.top = fldPosition.top + fld.offsetHeight + 'px'; //findPosY(fld)
    ...
    for (var i=1; i < parseInt(nlapiGetLineItemCount('item') + 1); i++)
    {
		...
		if ((linetype == 'InvtPart' || linetype == 'Assembly' || linetype == 'Kit') && !(createpo == 'DropShip' || createpo == 'SpecOrd') && createwo != 'T' && invtcommittable)
        {
            ...
            if (onhand < parseFloat(getEncodedValue('item',i,'quantity')))
            {
				...
				if (showedOnHandWarning[showedWarningId] == null && nlapiGetLineItemValue('item', 'itemsource', i) !== "PHANTOM")
				{
                    showedOnHandWarning[showedWarningId] = 'true';
                    warningMsg += getOnHandWarningMessage(locID, useloc, linetype, itemfldtext, onhand, unittext,
                        getEncodedValue('item',i,'backordered'), getEncodedValue('item',i,'onorder')) + '\n';
				}
			}
		}
	}
...
function initializeOrderAllocationStrategyForGroup(groupstart, groupend)
{
	for (var i=groupstart; i < groupend + 1; i++)
	{
		...
		if(hasAllocationStrategy) {
			...
			else if ('salesorder' == nlapiGetRecordType() && nlapiGetLineItemValue('item', 'defaultitemallocationstrategy', i) != '') {
				nlapiSetLineItemValue('item', 'orderallocationstrategy', i, nlapiGetLineItemValue('item', 'defaultitemallocationstrategy', i));
			}
			else {
...
function nlapiSetCurrentLineItemValueIfChanged(machine, field, value, firefieldchanged, synchronous)
{
	if (nlapiGetCurrentLineItemValue(machine, field) != value)
	{
		nlapiSetCurrentLineItemValue(machine, field, value, firefieldchanged, synchronous);
	}
}
...
function clearItemSourceField()
{
	...
	nlapiSetCurrentLineItemValueIfChanged('item', 'itemsource', item ? 'STOCK' : '', true, false)
}
...
function enableChargeFields()
{
    if (hasItemMachField('chargetype'))
    {
        ...
        var chargeType = getItemMachValue('chargetype'), isQuantityRateAmountDisabled = isdisabled;
		...
		if (isQuantityRateAmountDisabled)
		{
...
function canIgnoreLine()
{
	var idx = nlapiGetCurrentLineItemIndex('item');
	var count = nlapiGetLineItemCount('item');

	var isHiddenBufferLine = idx > count;
	var isMachineNotEditable = hasLegacyMachine() && !item_machine.allow_insert;

	return isHiddenBufferLine && isMachineNotEditable;
}
...
var autoLocationAssignmentModule = (function(){
	...
	{
		nlapiSetLineItemDisabled(itemMachine, noautoassignlocationField,
								 !isRelevantItemType(nlapiGetCurrentLineItemValue(itemMachine, itemTypeField)));
	};
	...
	var updateDoNotAutoAssignLocationWhenLocationChange = function (locationField)
	{
		...
		if (!isStorePickUpSelected)
		{
			if (isRelevantItemType(nlapiGetCurrentLineItemValue(itemMachine, itemTypeField)))
			{
				...
				{
					nlapiSetCurrentLineItemValue(itemMachine, noautoassignlocationField,
												 !!nlapiGetCurrentLineItemValue(itemMachine, locationField) ? 'T' : 'F', true, true);
				}
			}
		}
	};
...
var storePickUpConnector = (function ()
{
			...
			for (var i = 1; i <= count; i++)
			{
				if (isFulfillableLine(i))
				{
					if (nlapiGetLineItemValue(machineName, itemFulfillmentChoiceField, i) != value)
					{
						...
						nlapiSetCurrentLineItemValueIfChanged(machineName, fulfillmentLocationField, locationPrevValue, true, true);
		...
		if(getItemMachValue('item') != '') {
			...
			if (canHaveFulfillmentChoice(getItemType(), getItemMachValue('fulfillable'))) {
				if (currentLineFulfillmentChoice == '') {
					...
					nlapiSetCurrentLineItemValue(machineName, itemFulfillmentChoiceField, newLineFulfillmentChoiceValue, true, true);
				}
			} else {
				nlapiSetCurrentLineItemValue(machineName, itemFulfillmentChoiceField, '', true, true);
			}
		}
	};
	...
	var restoreItemMachLocationPrevValue = function ()
	{
		if (itemMachLocationPrevValue != '')
		{
			try
			{
				nlapiSetCurrentLineItemValueIfChanged(machineName, fulfillmentLocationField, itemMachLocationPrevValue, nlapiGetCurrentLineItemValue(machineName, fulfillmentLocationField) != itemMachLocationPrevValue, true);
			}
			catch (e)
			{
...
function updateBillableEnableFlag()
{
	if (!hasItemMachField('billableenabled'))
		return;
	var billableEnabled = nlapiGetCurrentLineItemValue('item', 'billableenabled') === 'T';
    var index = nlapiGetCurrentLineItemIndex('item');
    nlapiSetLineItemDisabled('item', 'isbillable', !billableEnabled, index);
}

function legacyUpdateBillableEnableFlag()
{
	if (!hasItemMachField('customer') || !hasItemMachField('isbillable'))
		return;
	if (nlapiGetContext().getPreference("DEFAULTITEMSBILLABLE") === 'T') {
		setItemMachValue('isbillable', (getItemMachValue('customer') > 0 ? 'T' : 'F'));
	} else {
		if (getItemMachValue('customer').length == 0) setItemMachValue('isbillable', 'F');
	}
}
...
var workOrderConnector = (function()
{
	...
	function handleItemSourceChange(lineId, itemHasChanged, origLineData)
	{
		...
		var unitConversionRate = parseFloatOrZero(getEncodedItemValue(lineId, 'unitconversionrate'));
        if (unitConversionRate === 0) unitConversionRate = 1;

		if (itemSource == 'PHANTOM' && (itemHasChanged || initItemSource != 'PHANTOM'))
		{
			setInventoryNumbersFieldsDisabled(true, lineId);

			var params = createParameterMap({item: itemId, unitconversionrate: unitConversionRate});
	...
	function markAllPhantom()
	{
		...
		for (var i = 1, count = nlapiGetLineItemCount('item'); i <= count; i++)
		{
			if (getEncodedItemValue(i, 'itemtype') == 'Assembly' && getEncodedItemValue(i, 'itemsource') != 'PHANTOM' && (parseFloatOrZero(getEncodedItemValue(i, 'quantityfulfilled')) == 0))
			{
				...
				var unitConversionRate = parseFloatOrZero(getEncodedItemValue(i, 'unitconversionrate'));
				if (unitConversionRate === 0) unitConversionRate = 1;
				unitConversionRates.push(unitConversionRate.toString());
		...
		var params = createParameterMap({items: items, unitconversionrates: unitConversionRates, expandassembly: 'T'});
	...
	{
		if (isInternalExecution || canIgnoreLine())
			return true;
