...
var callClientScript;
var callRecordScript;
var fetchClientScripts;

var global = getGlobalScope();
var nlapi = global.nlapi || {};
nlapi.assignDefineIfEmpty = true;
...
(function initScripting() {
	...
	var includeN = (function ()
	{
		var ranAlready = false;

		function doIncludeN(debug) {
			...
			try {
				 ...
				 */
				require(['N'], function () {});
				var dynamicData = require("N/dynamicData");
				dynamicData.init(_dynamicData);
				dynamicData.init(_dynamicUiModeSpecificData);
				require(['N/domCurrentRecord'], function (domCurrentRecord) {
					v2Record = domCurrentRecord.get();
				});
			}
			finally {
				require.setInternal(wasInternal);
				require.forceSync(wasSync);
			}
			v2Record.fetchClientScripts();
	...
	function stillLoadingScripts() {
		return !v2Record || v2Record.stillLoadingScripts();
	}

	callClientScript = (function () {

		function isNewUI() {
			return !!window && !!window.location && !!window.location.href && /([?&])withcurrentrecord\b/.test(window.location.href);
		}
		...
		function doCallClientScript(trigger, args) {
			...
			if (hasV2clientScripts()) {
				fetchClientScripts();
				return v2Record.runClientScript(trigger, args);
			}
			else {
				return oldScriptCall(trigger, args);
			}
		}
	...
	callRecordScript = function doCallRecordScript(options)
	{
		if (v2Record)
		{
			return v2Record.runRecordScript(options);
		}
		return 'legacyScript' in options ? options.legacyScript : true;
	};
	...
	fetchClientScripts = (function ()
	{
		...
		{
			return ranAlready && !stillLoadingScripts();
		}
	...
	function oldScriptCall(trigger, args)
	{
		...
		if ( (thisScriptList != null) && (thisScriptList.value.length > 0) )
		{
			...
			for (var i = 0; validationResultToBoolean(isValid) && i < scripts.length; i++)
			{
				if (typeof(NLRUM) !== 'undefined' && !isValEmpty(scripts[i]))
					NLRUM.clientScriptBegin(trigger, fScriptIds[i], args);
...
if (typeof require !== 'undefined' && typeof require.SSModuleLoader !== 'undefined') {
	require.SSModuleLoader.perfTracking.create('clientScriptTracking');
}

var triggerInfo = {
	pageInit: { name: 'pageInit', fieldId: "nlapiPI", needsReturn: false },
	saveRecord: { name: 'saveRecord', fieldId: "nlapiSR", needsReturn: true },
	lineInit: { name: 'lineInit', fieldId: "nlapiLI", needsReturn: false },
	validateDelete: { name: 'validateDelete', fieldId: "nlapiVD", needsReturn: true },
	validateInsert: { name: 'validateInsert', fieldId: "nlapiVI", needsReturn: true },
	validateLine: { name: 'validateLine', fieldId: "nlapiVL", needsReturn: true },
	postSourcing: { name: 'postSourcing', fieldId: "nlapiPS", needsReturn: false },
	recalc: { name: 'recalc', fieldId: "nlapiRC", needsReturn: false },
	validateField: { name: 'validateField', fieldId: "nlapiVF", needsReturn: true },
	fieldChanged: { name: 'fieldChanged', fieldId: "nlapiFC", needsReturn: false },
	localizationContextEnter: { name: 'localizationContextEnter', needsReturn: false },
	localizationContextExit: { name: 'localizationContextExit', needsReturn: false }
};
if (Object.freeze)
{
	triggerInfo = Object.freeze(triggerInfo);
}
// convert the extended validation return value to a boolean true/false
var validationResultToBoolean = function (x)
{
	return x === Object(x) && x.hasOwnProperty('valid') ? !!x.valid : !!x;
};

// if it is a validation result object, keep as is, otherwise align to boolean true/false
var alignValidationResult = function (x)
{
	return x === Object(x) && x.hasOwnProperty('valid') ? x : !!x;
};

// handle the extended validation return value in a backward compatible way
var handleValidationResult = function (x)
{
	if (x !== Object(x) || !x.hasOwnProperty('valid'))
		return x;
	if (x.valid === false && x.messages && x.messages.length)
		alert(x.messages.join('\\n'));
	return x.valid;
};

function nlapiPageInit(type)
{
	//Wait for async scripts to load before proceeding with pageInit
	if(includedVersion2plusScript() && window && window.setTimeout && !fetchClientScripts()){
		window.setTimeout(nlapiPageInit, 0, type);
		return;
	}
    ...
    try
    {
        ...
        {
            nsapiCallScript( triggerInfo.pageInit.name, "internal", document.forms['main_form'].elements.wfPI.value, [type] );
        }
        if (document.forms['main_form'].elements.nsapiPI != null && document.forms['main_form'].elements.nsapiPI.value.length > 0)
        {
            nsapiCallScript( triggerInfo.pageInit.name, "internal", document.forms['main_form'].elements.nsapiPI.value, [type] );
        }
        callClientScript(triggerInfo.pageInit.name, [type]);
    }
...
function nlapiSaveRecord()
{
    ...
    {
        isValid = nsapiCallScript(triggerInfo.saveRecord.name, "internal", document.forms['main_form'].elements.wfSR.value);
        if (!isValid)
            return false;
    }
    if (document.forms['main_form'].elements.nsapiSR != null && document.forms['main_form'].elements.nsapiSR.value.length > 0)
    {
        isValid = handleValidationResult(nsapiCallScript(triggerInfo.saveRecord.name, "internal", document.forms['main_form'].elements.nsapiSR.value));
        if (!isValid)
            return false;
    }
    isValid = isValid && handleValidationResult(callClientScript(triggerInfo.saveRecord.name));
    return isValid;
}
...
function nlapiValidateField(type, fldnam, linenum, column, optionswin)
{
    ...
    try
    {
        ...
        {
            isValid = nsapiCallScript( triggerInfo.validateField.name, "internal", document.forms['main_form'].elements.wfVF.value, [type, fldnam, linenum, column] );
            if (!isValid)
                return false;
        }
        if (document.forms['main_form'].elements.nsapiVF != null && document.forms['main_form'].elements.nsapiVF.value.length > 0)
        {
            isValid = handleValidationResult(nsapiCallScript(triggerInfo.validateField.name, "internal", document.forms['main_form'].elements.nsapiVF.value, [type, fldnam, linenum, column]));
            if (!isValid)
                return false;
        }
        isValid = isValid && handleValidationResult(callClientScript(triggerInfo.validateField.name, [type, fldnam, linenum, column]));
        return isValid;
    }
    finally
    {
...
function nlapiFieldChanged(type, fldnam, linenum, column, optionswin)
{
    ...
    {	/* --- set inited flags so that machine calls function correctly. */
        ...
        {
            nsapiCallScript( triggerInfo.fieldChanged.name, "internal", document.forms['main_form'].elements.wfFC.value, [type, fldnam, linenum, column] )
        }
        if (document.forms['main_form'].elements.nsapiFC != null && document.forms['main_form'].elements.nsapiFC.value.length > 0)
        {
            nsapiCallScript( triggerInfo.fieldChanged.name, "internal", document.forms['main_form'].elements.nsapiFC.value, [type, fldnam, linenum, column] )
        }
...
{
    callClientScript(triggerInfo.fieldChanged.name, [type, fldnam, linenum, column]);
}
...
function nlapiPostSourcing(type, fldnam, linenum)
{
    ...
    {	/* --- set inited flags so that machine calls function correctly. */
        ...
        {
            nsapiCallScript( triggerInfo.postSourcing.name, "internal", document.forms['main_form'].elements.wfPS.value, [type, fldnam, linenum] )
        }
        if (document.forms['main_form'].elements.nsapiPS != null && document.forms['main_form'].elements.nsapiPS.value.length > 0)
        {
            nsapiCallScript( triggerInfo.postSourcing.name, "internal", document.forms['main_form'].elements.nsapiPS.value, [type, fldnam, linenum] )
        }
        callClientScript(triggerInfo.postSourcing.name, [type, fldnam, linenum]);
    }
...
function nlapiLineInit(type)
{
    try
    {
        ...
        {
            nsapiCallScript( triggerInfo.lineInit.name, "internal", document.forms['main_form'].elements.nsapiLI.value, [type] )
        }
        callClientScript(triggerInfo.lineInit.name, [type]);
    }
    finally
    {
        optwin = null;
...
function nlapiValidateDelete(type, lineCount)
{
    try
    {
        var isValid = true;
        ...
        {
            isValid = handleValidationResult(nsapiCallScript(triggerInfo.validateDelete.name, "internal", document.forms['main_form'].elements.nsapiVD.value, [type]));
        }
        isValid = isValid && handleValidationResult(callClientScript(triggerInfo.validateDelete.name, [type, lineCount]));
        return isValid;
    }
    finally
    {
...
function nlapiValidateInsert(type)
{
    try
    {
        ...
        {
            isValid = handleValidationResult(nsapiCallScript(triggerInfo.validateInsert.name, "internal", document.forms['main_form'].elements.nsapiVI.value, [type]));
        }
        isValid = isValid && handleValidationResult(callClientScript(triggerInfo.validateInsert.name, [type]));
        return isValid;
    }
    finally
    {
...
function nlapiValidateLine(type)
{
    try
    {
        ...
        {
            isValid = handleValidationResult(nsapiCallScript(triggerInfo.validateLine.name, "internal", document.forms['main_form'].elements.nsapiVL.value, [type]));
            if (!isValid)
                return false;
        }
        isValid = isValid && handleValidationResult(callClientScript(triggerInfo.validateLine.name, [type]));
        return isValid;
    }
    finally
    {
...
function nlapiRecalc(type, localRecalc, operation)
{
    try
    {
        ...
        {
            nsapiCallScript( triggerInfo.recalc.name, "internal", document.forms['main_form'].elements.nsapiRC.value, [type, operation] )
        }
        callClientScript(triggerInfo.recalc.name, [type, operation]);
    }
    finally
    {
        optwin = null;
        ...
        if (nsapiIsInternal() || unsupported.indexOf(' '+getFieldName(fldnam)+' ') == -1)
        {
            ...
            if ( fld != null )
            {
                ...
                try
                {
					...
					var valField = document.getElementById(fldnam+"_val");
                    if (valField != null)
                    {
						...
						isInlineHTMLField(fld) ? setInlineTextValue(valField, displayValue) : setTextValue(valField, displayValue);
                    }
            ...
            else if (document.getElementById(fldnam+"_val") != null)	// handle DISPLAYONLY|COMPUTED or inlinehtml fields */
            {
                ...
                if (isNumericField(fld) || isCurrencyField(fld))
					displayValue = NLNumberToString(value);

				isInlineHTMLField(fld) ? setInlineTextValue(fld, displayValue) : setTextValue(fld, displayValue);
            }
            else
            {
            ...
            {   /* strip out user-specified trailing slash or wildcard whenever applicable. */
                ...
                try
                {
                    if (synchronous)
                        setSlavingAsync(false);
                    nsapiFireOnChange(fld, firefieldchanged, strictFireFieldChanged);
	                if (strictFireFieldChanged && !firefieldchanged && !startsWithSearch) { // strictFireFieldChanged===true => it is v2
		                var enableValueLookup = typeof _dynamicRecordSpecificData !== 'undefined' && _dynamicRecordSpecificData.hasOwnProperty("_canary") ?
			                    !!_dynamicRecordSpecificData._canary["RARS_ENABLE_VALUE_LOOKUP_FOR_SETTEXT"] : false;
		                var textToVal = enableValueLookup && NLEntryForm_querySelectValue(fldnam, null, txt);
		                if (textToVal) {
			                var fldHidden = getFormElement(form, getFieldName(fldnam))
			                syncpopup(fldHidden, textToVal[txt], txt);
		                }
	                }
                }
                finally
        ...
        if (hasEncodedField(type, fldnam))
        {
            ...
            try
            {
                ...
                if (isPopupSelect(fld) || isPopupMultiSelect(fld))
                {
	                ...
	                text = getDropdown(fld).getTextForValue(value);
                }
...
function nlapiGetCurrentLineItemValue(type,fldnam)
{
    ...
    if (hasEncodedField(type, fldnam))
    {
        ...
        else
        {
            ...
            var element = getFormElement( document.forms[type.toLowerCase()+'_form'], fldnam);
            if (!element && (typeof __CANARIES !== "undefined" && __CANARIES.RARS_CURRENT_LINE_VALUE_FOR_VIEW_LIST_ELEMENTS === true))
            {
                // the element apparently exists only for those fiels that are editable and not for simple labels
                return nlapiGetLineItemValue(type, origFldNam, linenum);
            }
            return getFormValue(element);
        }
    }
    else if (hasEncodedField(type, "options"))
    {
        ...
        var options = document.forms[type.toLowerCase()+'_form'].options;
        return options ? getnamevaluelistvalue(options.value, fldnam) : null;
    }
    else
    {
        if (typeof fieldtosubrecordmap != 'undefined')
...
function nlapiGetCurrentLineItemText(type,fldnam)
{
    ...
    else if (hasEncodedField(type, "options"))
    {
        ...
        var options = document.forms[type.toLowerCase()+'_form'].options;
        return options ? getnamevaluelistdisplayvalue(options.value, fldnam) : null;
    }
    else
        return null;
}
...
function nlapiGetField(fldnam)
{
    ...
    var fldObj = typeof(ftabs) != 'undefined' && ftabs[getFieldName(fldnam)] != null ? new nlobjField(getFieldName(fldnam), ftypes[getFieldName(fldnam)] != null ? ftypes[getFieldName(fldnam)] : 'text') : null;
...
function nlapiGetSubList(type, listIncluded)
{
    if ( hasLineItemGroup(type) || (listIncluded === true && document.getElementById(type+'_layer') != null))
    {
...
function nlapiEncrypt(s, type, key)
{
    ...
    if (type == null)
        type = "sha1";
    return nsServerCall(getContextUrl(nsJSONProxyURL), "encrypt", [s, type, key != null ? key : null], null, 'POST');
}
function nlapiDecrypt(s, type, key)
{
    nsapiCheckArgs( [s], ['encryptedtext'], 'nlapiDecrypt' );
    if ( type == "aes" || type == "base64" || type == "xor" )
        return nsServerCall(getContextUrl(nsJSONProxyURL), "decrypt", [s, type, key != null ? key : null], null, 'POST');
    return null;
}
function nlapiExchangeRate(fromCurrency, toCurrency, effectiveDate)
{
    ...
    var rate = nsServerCall(getContextUrl(nsJSONProxyURL), "exchangeRate", [fromCurrency, toCurrency, effectiveDate != null ? effectiveDate : nlapiDateToString(new Date())]);
...
function nlapiVoidTransaction (type, id)
{
    ...
    nsapiCheckUsage( );
    return nsServerCall(getContextUrl(nsJSONProxyURL), 'voidTransaction', [type, id]);
}
...
function nlapiSubmitField(type, id, fields, values, options)
{
    ...
    try
    {
        ...
        if (fields instanceof String || typeof(fields) == 'string')
        {
	        ...
	        {
		        nsSetChildValues(nsField, "value", values);
	        }
	        else
	        {
        ...
        else
        {
            for (var i = 0; fields != null && i < fields.length; i++)
            {
	            ...
	            if (values != null)
                {
	                ...
	                {
		                nsSetChildValues(nsField, "value", values[i]);
	                }
	                else
	                {
		                nsSetChildValue(nsField, "value", values[i]);
	                }
                }
	            else
                {
                    nsSetChildValue(nsField, "value", null);
	            }
            }
        }
...
function nlapiDeleteRecord(type, id)
{
    ...
    try
    {
        nsServerCall(getContextUrl(nsJSONProxyURL), "deleteRecord", [type, id]);
...
function nlapiGetSearchColumns(type, filter, op)
{
    ...
    try
    {
        var rawResults = nsServerCall(getContextUrl(nsJSONProxyURL), 'getSearchColumns', [type, filter, op]);
...
function nlapiSearchRecord(type, id, filtersOrExpression, columns)
{
    ...
    try
    {
        ...
        var rawResults = nsServerCall(getContextUrl(nsJSONProxyURL), 'searchRecord', [type, id, rawFilters, rawColumns], null, 'POST');
...
function nsapiParseSearchFilterExpression(filterExpression)
{
    ...
    var payloadMap = nsServerCall(getContextUrl(nsJSONProxyURL), 'parseSearchFilterExpression', [marshaled], null, 'POST');
...
function nlapiSearchGlobal(keywords)
{
    ...
    try
    {
        var rawSearchResults = nsServerCall(getContextUrl(nsJSONProxyURL), 'searchGlobal', [keywords]);
...
function nlapiSearchDuplicate(type, fields, id)
{
    ...
    try
    {
        ...
        var rawSearchResults = nsServerCall(getContextUrl(nsJSONProxyURL), 'searchDuplicate', [type, isNaN(parseInt(id)) ? -1 : parseInt(id), obj]);
        var rowResults = nsapiExtractSearchResults( rawSearchResults );
...
function nlapiResolveURL(type, identifier, id, pagemode)
{
    ...
    try
    {
        ...
        var sUrl = nsServerCall(getContextUrl(nsJSONProxyURL), "resolveURL", [type, identifier, id, pagemode]);
        return sUrl;
    }
    catch( e )
    {
...
function nlapiAttachRecord(type, id, type2, id2, attrs)
{
    ...
    try
    {
        nsServerCall(getContextUrl(nsJSONProxyURL), "attachRecord", [type, id, type2, id2, nsapiExtractMap(attrs)]);
...
function nlapiDetachRecord(type, id, type2, id2, attrs)
{
    ...
    try
    {
        nsServerCall(getContextUrl(nsJSONProxyURL), "detachRecord", [type, id, type2, id2, nsapiExtractMap(attrs)]);
    ...
    try
    {
        ...
        else
        {
	        ...
	        if (!!window.NLScriptIdForLogging && !!window.NLDeploymentIdForLogging)
	            nsServerCall(getContextUrl(nsJSONProxyURL, window.NLScriptIdForLogging, window.NLDeploymentIdForLogging), "logExecutionWithDeployment", [window.NLScriptIdForLogging, window.NLDeploymentIdForLogging, type, title, details != null ? details.toString() : null]);
	        else if (!isValEmpty(scriptid) && scriptid != "customform")
		        nsServerCall(getContextUrl(nsJSONProxyURL, scriptid), "logExecution", [scriptid, nlapiGetRecordType(), type, title, details != null ? details.toString() : null]);
        }
    }
    catch( e )
    {
...
function nlapiChangeCall(params)
{
	...
	//Check if different base URL should be used for this change call than document location
	for ( var param in params )
	{
		if (param === 'base_url_for_change_call' && params.hasOwnProperty(param))
			url = params[param];
	}

	//Process parameters to be updated in URL
    for ( var param in params )
    {
	    if (param != 'base_url_for_change_call') {
...
nlobjRecord.prototype.getDateTimeValue = function (fldname, timezone)
{
    if (timezone == null)
        ...
        if (preferredTimeZone == timezone)
            ...
            return nsServerCall(getContextUrl(nsJSONProxyURL), 'calculateGetDateTimeWithTimeZone', [storedDateTime, timezone]);
    }
}
...
nlobjRecord.prototype.setDateTimeValue = function (fldname, value, timezone)
{
    ...
    else
    {
        ...
        {
            var newVal = nsServerCall(getContextUrl(nsJSONProxyURL), 'calculateSetDateTimeWithNewTimeZone', [value, timezone]);
...
{
    var payloadMap = nsServerCall(getContextUrl(nsJSONProxyURL), 'loadSearch', [this.type, this.searchId]);
...
nlobjSearch.prototype.saveSearch = function(title, scriptId)
{
    try
    {
        ...
        var searchId = nsServerCall(getContextUrl(nsJSONProxyURL), 'saveSearch', [title, scriptId, this.type, this.searchId, rawFilters, rawColumns, this.isPublic], null, 'POST');
...
nlobjSearch.prototype.deleteSearch = function()
{
    try
    {
        ...
        nsServerCall(getContextUrl(nsJSONProxyURL), 'deleteSearch', [this.type, this.searchId]);
//		nsapiLogUsage( 'nlapiSearchRecord', isValEmpty(searchId) && nsapiIsLookup(this.filters) ? type : null );
    }
    catch (e)
    {
...
{
    var rawFilters = nsapiMarshalSearchFilters(this.filters);
    var payload = nsServerCall(getContextUrl(nsJSONProxyURL), 'buildSearchFilterExpression', [rawFilters], null, 'POST');
...
nlobjSearch.prototype.setRedirectURLToSearch = function()
{
    try
    {
        var rawFilters = nsapiMarshalSearchFilters(this.filters);
        var rawColumns = nsapiMarshalSearchColumns(this.columns);
        var url = nsServerCall(getContextUrl(nsJSONProxyURL), 'prepareSearchPage', [this.type, this.searchId, rawFilters, rawColumns]);
        nsapiSetRedirectURL(url);
    }
    catch( e )
    {
...
nlobjSearch.prototype.setRedirectURLToSearchResults = function()
{
    try
    {
        var rawFilters = nsapiMarshalSearchFilters(this.filters);
        var rawColumns = nsapiMarshalSearchColumns(this.columns);
        var url = nsServerCall(getContextUrl(nsJSONProxyURL), 'prepareSearchResults', [this.type, this.searchId, rawFilters, rawColumns]);
        nsapiSetRedirectURL(url);
    }
    catch( e )
    {
...
nlobjSearch.prototype._getResultsSlice = function(start, end)
{
    try
    {
        var rawFilters = nsapiMarshalSearchFilters(this.filters);
        var rawColumns = nsapiMarshalSearchColumns(this.columns);
        var rawResults = nsServerCall(getContextUrl(nsJSONProxyURL), 'searchRecordSlice', [this.type, this.searchId, rawFilters, rawColumns, start, end]);
...
nlobjContext.prototype.getBundleId = function( ) { return fBundleIds[this.getScriptId()]; }
nlobjContext.prototype.getScriptId = function( ) { return _getCurrentScriptId(); }
...
nlobjContext.prototype.getDeploymentId = function( ) { return fDeployIds[this.getScriptId()]; }
nlobjContext.prototype.getScriptType = function( ) { return "CLIENT"; }

nlobjContext.prototype.getFeature = function( name )
{
	if(name == null) return false; //maintaining old behavior.
	name = name.toUpperCase();
	//Verify that the features global object has been already loaded
	if(typeof featuresCache === "undefined" || !featuresCache)
	{
		//cache miss or empty cache, load all features from the server into the global variable that's acting as a cache.
		nlapiLogExecution("DEBUG", "getFeature:: cache miss, loading cache...");
		nsServerCall(nsJSONProxyURL, "getFeatures", [], function(data, err){
			if(err != null || err) {
				// Log error
				nlapiLogExecution("ERROR", "Error in callback function while calling getFeatures()");
			} else {
				featuresCache = data.set;
			}
		});
		return nsServerCall(nsJSONProxyURL, "getFeature", [name]);
	}
...
nlobjContext.prototype.getPreference = function( name )
{
	...
	if(typeof preferencesCache === "undefined" || !preferencesCache ||typeof preferencesCache[name] === "undefined")
	{
		...
		nlapiLogExecution("DEBUG", "getPreference:: cache miss, loading cache...");
		nsServerCall(nsJSONProxyURL, "getPreferences", [], function(data, err){
			if(err != null || err) {
				// Log error
				nlapiLogExecution("ERROR", "Error in callback function while calling getPreferences()");
			} else {
				preferencesCache = data;
			}
		});
		// After triggering the async call, return a single preference the same old way, this way will maintain the old behavior.
		// One of the main reasons for this, is that getPref is called synchronously on page_init, it's faster to return a single item and trigger the cache loading logic.
...
{
	name = name.toUpperCase();
	//Verify that the getPermissions global object has been already loaded
	if(typeof permissionsCache === "undefined" || !permissionsCache ||typeof permissionsCache[name] === "undefined")
	{
		//cache miss or empty cache.
		nlapiLogExecution("DEBUG", "getPermission:: cache miss, loading cache...");
		nsServerCall(nsJSONProxyURL, "getPermissions", [], function(data, err){
			if(err != null || err) {
				// Log error
				nlapiLogExecution("ERROR", "Error in callback function while calling getPermissions()");
			} else {
				permissionsCache = data;
			}
		});
		// While cache is being loaded, return a single result maintaining the old behavior.
		return nsServerCall(nsJSONProxyURL, "getPerm", [name]);
	}
	// return preference value from cached object.
...
function nlobjSubList( name )
{
    ...
    this.type = thisType;
    this.label = null;
	this.hidden = document.getElementById(this.name+"_layer") == null;

	var sufix = "";
	if (document.getElementById(name + '_pane') != null) {
		sufix = '_pane'; //unlayered mode
	} else if (document.getElementById(name + 'lnk') != null) {
		sufix = 'lnk'; //layered tab header
	} else if (document.getElementById(name + '_layer') != null) {
		sufix = '_layer'; //layered tab content
	}
	this.display = !this.hidden ? document.getElementById(name + sufix).style.display !== 'none' : false;
}
...
function nsapiCallUserScript(trigger, scriptid, func, args)
{
    ...
    try
    {
	    ...
	    if (includedVersion2plusScript())
	    	require('N/internal/reflet').setupScript({scriptId: scriptid});
...
{
	var run = includedVersion2plusScript()
		? function run(callback)
		{
			var contextSwitch = require('N/contextSwitch');
			var domCurrentRecord = require('N/domCurrentRecord');

			var scriptInfo = {scriptId: scriptid};
			if (scriptid !== undefined && scriptid !== 'internal' && hasV2clientScripts())
			{
				var superScriptList = _clientScriptContext.superScriptList;
				for (var i = 0; i < superScriptList.length; i++)
				{
					var moduleInfo = superScriptList[i];
					if (moduleInfo.scriptId === scriptid)
					{
						scriptInfo = moduleInfo;
						break;
					}
				}
			}
			...
			var context = {
				record: domCurrentRecord.get(),
				scriptInfo: scriptInfo,
				nlrumArgs: [trigger, scriptid, args, scriptInfo.version || '1.0', nlapiGetRecordType()]
			};
			if (typeof(nlapi) !== 'undefined' && nlapi && nlapi.async) nlapi.async.prepareForAsync({id: scriptid, trigger: trigger, args: args});
			try
			{
				return contextSwitch.execute(context, callback);
			}
			finally
			{
...
function nsapiQueryScript(arg) {
	var c = 0;
	var scriptId = _getCurrentScriptId();
	var functionName;
	// In strict mode, arguments.callee.caller is inaccessible and throws an exception in IE and FF.
	try {
...
function nsapiUpdateMachines()
{
    for( var s in nsUpdatedMachines )
    {
        ...
        {
        	var mchname = String(s) + '_machine';
            var mch = eval('typeof ' + mchname + ' !== "undefined" ? ' + mchname + ' : null');
            if (mch != null)
                mch.buildtable();
        }
    }
...
var nsTasklinks;
var nsRecordTypes;
var nsRecordTypesAreEnumerable;
...
function nsapiInitRecords()
{
    if ( nsRecordTypes == null )
    {
        ...
        nsRecordTypesAreEnumerable = Object.keys(nsRecordTypes).length > 0;
    }
}
function nsapiGetRecord(type)
{
    ...
    var out = nsRecordTypes[ typeLC ];
    if (out == null && !nsRecordTypesAreEnumerable)
    {
        out = nsRecordTypes[ typeLC ] = nsServerCall(nsJSONProxyURL, "getRecordType", [ typeLC ]);
    }
    return out;
}
...
function nsapiAjaxResponse( response, callbackFunc )
{
    ...
    var nlheaders = [];
	if (headers[nsHeaderPrefix+'-Code'] != null || headers[nsHeaderPrefix.toLowerCase()+'-code'] != null)  /* handle proxyed requests */
	{
		var prefix = headers[nsHeaderPrefix+'-Code'] != null ? nsHeaderPrefix : nsHeaderPrefix.toLowerCase();
		code = headers[nsHeaderPrefix+'-Code'] || headers[nsHeaderPrefix.toLowerCase()+'-code'];
		...
		for ( var header in headers )
			if(headers.hasOwnProperty(header))
				if ( header.indexOf( prefix ) == 0 )
					nlheaders[header.substring( prefix.length+1 )] = headers[header];
	}
    else
        nlheaders = headers;
...
function getFieldName(fldname)
{
    ...
    if (type == 'userpreferences'
        || type == 'setupcompany'
        || type == 'acctsetup'
        || type == 'duplicatedetectsetup'
        || (type === 'subsidiary' && fldname.toLowerCase() === 'calendarsystem')
        || type === 'intercompanypreferences')
        return fldname;
    else
...
function addScriptContext(request)
{
	var scriptId = _getCurrentScriptId();
	if (scriptId)
	{
		request.setAttribute("script", scriptId);
		try
		{
			request.setAttribute("deploy", fDeployIds[scriptId]);
		}
		catch (e) {
			//fDeployIds effectively undefined (this can happen in VirtualBrowser mode)
		}
	}
}

function getContextUrl(url, script, deploy)
{
	var scriptId = script ? script : _getCurrentScriptId();
	if (scriptId)
	{
		url += "?script=" + scriptId;
		if (deploy)
		...
		else
		{
			try
			{
				url += "&deploy=" + fDeployIds[scriptId];
			}
			catch (e) {
				//fDeployIds effectively undefined (this can happen in VirtualBrowser mode)
			}
...
{
    if (typeof callRecordScript === 'undefined') // TODO TAX MACRO: need a check that transaction is taxable
        throw nlapiCreateError('SSS_UNSUPPORTED_METHOD', 'Unsupported method');

    try
    {
	    callRecordScript({eventName: 'calculateTax', isCustomMethodCall: true});
    }
    catch (e)
    {
        throw nlapiCreateError(e);
...
function ()
{
	...
	function RemoteRecordDelegate(recordDelegate)
	{
		...
		function setSelectValue(params, value, text, fireFieldChange)
		{
			if(!value && value !== '')
			{
				if(params.isMachineField === true)
					...
					record.setText({fieldId: params.fieldName, text: text, fireFieldChange: fireFieldChange, noslaving: true});
			}
			else
		...
		this.setFavorites = recordUtil.setFavorites;
	...
	function BrowserRecordDelegate()
	{
		...
		function getFormForMachineOrField(machineName, fieldName) {
			if (!machineName) {
				return typeof (ftabs) != 'undefined' && ftabs[getFieldName(fieldName)] != null ? document.forms[ftabs[getFieldName(fieldName)] + '_form'] : document.forms[0];
			}
			return document.forms[machineName+'_form'];
		}
		function insertSelectOption(fieldName, value, text)
		{
			var form = getFormForMachineOrField(null, fieldName);
			doInsertSelectOption(form, fieldName, text, value);
		}
		function insertLineItemOption(machineName, fieldName, value, text)
		{
			var form = getFormForMachineOrField(machineName, fieldName);
			doInsertSelectOption(form, fieldName, text, value);
		}
		...
		function setFavorites(machineName, fieldName, favorites) {
			var form = getFormForMachineOrField(machineName, fieldName);
			var fld = getFormElement(form, getFieldName(fieldName));
			if (fld != null) {
				if (favorites != null) {
					setFavoriteOptions(fld, favorites);
				}
			}
		}
	...
	function getQueryRequest(metadata, masterInfo, recordDelegate)
	{
		...
		var sublistName = masterInfo.sublistName || masterInfo.sublistId;
	...
	/**
	 * static utility function that process the slavingResponse data in the json format and sets the data in the DOM using Nlapi
	 * when possible.
	 * @param slavingValues
	 * @param record
	 */
	...
	function applyFavorites(favoriteResults, record) {
		if (favoriteResults) {
			var RecordDelegate = (record) ? new RemoteRecordDelegate(record) : new BrowserRecordDelegate();
			Object.keys(favoriteResults).forEach(function (id) {
				var favObj = favoriteResults[id];
				var favorites = favObj[SlaveResultKey.OPTIONS];
				if (favorites) {
					RecordDelegate.setFavorites(favObj[SlaveResultKey.MACHINE_NAME] || null, favObj[SlaveResultKey.NAME], transformFavorites(favorites));
				}
			});
		}
	}
	...
	function applySlaveValueToRecord(RecordDelegate, slaveResult, fieldInfo)
	{
		...
		var favorites = slaveResult[SlaveResultKey.FAVORITES];
		if (favorites) {
			RecordDelegate.setFavorites(fieldInfo.machineName, fieldInfo.fieldName, transformFavorites(favorites));
		}
...
var __CANARIES = {
    RARS_CURRENT_LINE_VALUE_FOR_VIEW_LIST_ELEMENTS: false,
};
