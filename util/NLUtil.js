...
// - Glue needed to work with the page generator
...
// THIS IS COPY OF ORIGINAL NLUtil.jsp, ONLY SMALL CHANGES WERE APPLIED TO KEEP IT WORKING!!
...
// HACK before adding NS.Translations into iFrame
function fireAlertWithTranslation(key, params, concatValue) {
	var valueToConcat = concatValue ? concatValue : '';
	if (NS.Translations) {
		return NS.UI.Util.alertPromise(NS.Translations.getTranslatedString(key, params) + valueToConcat);
	} else {
		return NS.UI.Util.alertPromise(parent.NS.Translations.getTranslatedString(key, params) + valueToConcat);
	}
}
...
function getDocumentClientHeight()
{
	...
	if (NS.UI.Util.isSafari || NS.UI.Util.isChrome) {
		return (isWindowContainedInDivFrame ? parent.NS.Window.getActive().content.clientHeight : window.innerHeight);
	} else {
		return (isWindowContainedInDivFrame ? parent.NS.Window.getActive().content.clientHeight : jQuery(document).height());
	}
}
...
function getDocumentClientWidth()
{
	...
	if (NS.UI.Util.isSafari || NS.UI.Util.isChrome) {
		...
		return (isWindowContainedInDivFrame ? parent.NS.Window.getActive().content.clientWidth : jQuery(document).width());
	}
}
...
function scrollList(element){
	scrollDiv();
	var listTitle = document.querySelector('.uir-list-title');
	if (listTitle) {
		var hiddenHeight = element.scrollHeight - element.offsetHeight;
		var listTitleHeight = listTitle.offsetHeight;
		var titleVisible = (hiddenHeight > listTitleHeight) && element.scrollTop > 0;
		listTitle.classList.toggle('uir-list-title--collapsed', titleVisible);
	}
}
...
function getVisibleWindowHeight()
{
    ...
    return (isWindowContainedInDivFrame ? parent.NS.Window.getActive().content.contentWindow.innerHeight : jQuery(window).height());
}
...
function hideInvisibleRows()
{
    ...
    if (hiddenHeight > 0)
    {
        ...
        div.style.height = div.offsetHeight - hiddenHeight + (NS.UI.Util.isIE ? 0 : 28) + "px";
    }
}
...
{
    var n = 0, spaceIdx= 0;
	var dateformat = window.dateformat.replace(/f[mx]|"/gi, '');
    if (dateformat == "DD de MONTH de YYYY") {
...
function hasV2clientScripts()
{
	return (typeof(require) !== 'undefined') &&
		(typeof(_clientScriptContext) !== 'undefined') && (_clientScriptContext.superScriptList.length > 0);
}
function includedVersion2plusScript()
{
	return hasV2clientScripts() || hasV2RecordOwnerEventHandler();
}

function hasV2RecordOwnerEventHandler()
{
	return (typeof(require) !== 'undefined') && (typeof(_eventHandlerModules) !== 'undefined') && (_eventHandlerModules.length > 0);
}

function _getCurrentScriptId()
{
	try {
		return includedVersion2plusScript() ? require('N/internal/reflet').getScript().scriptId : window.NLScriptId;
	} catch (e) {
		// in case this was called from script body, script id is undefined
	}
}

// convert extended validation result to boolean
function getBooleanValidationResult(x)
{
	return (x === Object(x) && x.hasOwnProperty('valid')) ? x.valid : x;
}

function makeValidationQuirkLog(type, value, description, id)
{
    var fieldId = id || "";
    var activeScript = _getCurrentScriptId() || "(UI)";
...
function _doValidate(options)
{
	...
	return validator.validateField(fieldId, type, value, false, false, null, minVal, maxVal, undefined, mandatory, scale);
}

function validate_field(field, type, doalert, autoplace, minval, maxval, mandatory, separator, scale)
{
    if ((typeof validationFlowSwitch !== "undefined") && validationFlowSwitch.usesNewPath && (typeof require !== "undefined") && checkForModuleDependency())
    {
        if (field.value !== undefined  && field.value !== "")
        {
            ...
            try
            {
	            _doValidate({fieldId: field.name, type: type, value: validateMe, minVal: minval, maxVal: maxval, mandatory: mandatory, scale: scale});
	            isValid = true;
            }
            catch (e)
            {
    ...
    {
        return old_validate_field(field, type, doalert, autoplace, minval, maxval, mandatory, separator, scale);
    }
}
...
function setDefaultIndex(sel, val) {
	if (isNLDropDown(sel))
	{
...
function syncpopup(list,val,name,makedefault)
{
    ...
    else if ( isPopupSelect( list ) )
    {
        ...
        {
        	dispfld.value = name;
	        dispfld.setAttribute('previousvalue', name);
        	if (makedefault)
        ...
        {
            dispfld.value = '';
	        dispfld.setAttribute('previousvalue', '');
            if (makedefault)
...
function getSelectValue(sel)
{
	...
	{
		return (sel.options.length === 0 || sel.selectedIndex === -1 || sel.selectedIndex >= sel.options.length)
			? ''
	...
	if (isNLDropDown(sel))
	{
		return getDropdown(sel).getValue();
	}
	if (isNLMultiDropDown(sel))
	{
		return getMultiDropdown(sel).getSelectedValues();
	}
...
function setFavoriteOptions(select, favorites, win)
{
	if (!isBackend)
	{
		if (isNLDropDown(select))
		{
			var dd = getDropdown(select, win);
			if (dd == null)
			{
				return;
			}

			dd.setFavorites(favorites);
		}
		else if (isNLMultiDropDown(select))
		{
			throw "Unsupported operation: Adding options to a Multiple dropdown"
		}
		else if (isPopupSelect(select))
		{
			throw "Unsupported operation: Adding options to a popup select"
		}
		else
		{
			throw "Unsupported operation: Adding options to a static select"
		}
	}
}
...
function setSelectValue(sel, val)
{
    ...
    else if (isNLDropDown(sel))
    {
        ...
        if(dd.isOpen)
        {
			dd.setCurrentCellInMenuOnIndex(idx);
		}
        ...
        if (trElem)
        {
        	if (val)
        	{
        		...
        		if (trElem.className.indexOf('Dis') < 0)
				{
			        ...
			        trElem.classList.replace('pgBntG', 'pgBntGDis');
			        trElem.classList.replace('pgBntY', 'pgBntYDis');
		        }
        	}
        	else
			{
		        trElem.className = trElem.className.replace(/Dis/g, '');
	        }
        }
    }
}
...
{
	return elem && (elem.tagName == "INPUT" || elem.tagName == "BUTTON") && (elem.type == "submit");
}
...
{
	return sel != null && sel.className && sel.className.indexOf('nldropdown') >= 0;
}
...
function setNLCheckboxDisabled( fld, bDisabled)
{
    ...
    if (span) {
	    span.className = sClassName + " uir-field-input";
    }
}
...
function NLCheckboxOnClick(span)
{
	...
	if(!NS.UI.Util.isRedwood){
		...
		// In redwood, clicking works as expected and changes the value directly
		if (inpt.type == 'checkbox' && inpt.checked) {
			span.className = span.className.replace('_unck', '_ck');
		}
		else {
...
function setNLSummaryFieldDisabled( fld, bDisabled)
{
    ...
    if (helperLink != null) {
        helperLink.style.visibility = bDisabled ? "hidden" : (NS.UI.Util.isRedwood ? "" : "inherit");
	}
}
...
function isCurrencyField( fld )
{
    ...
    if (dataType == "currency" || dataType == "poscurrency" || dataType == "currency2" || dataType == "nonnegcurrency")
        return true;
    return false;
}
...
function isNumericField( fld )
{
    ...
    if(typeof fld.getAttribute != 'undefined')
        dataType = fld.getAttribute("dataType");
    if (dataType == "float" || dataType == "posfloat" || dataType == "nonnegfloat" || dataType == "integer" || dataType == "posinteger" || dataType == "rate" || dataType == "ratehighprecision" || dataType == "percent" || dataType == 'dynamicprecision')
        return true;
    return false;
}

function isInlineHTMLField(fld) {
	return fld && typeof fld.getAttribute != 'undefined' && fld.getAttribute('dataType') === 'inlinehtml';
}
...
function setNLNumericValue(fld, value)
{
    ...
    if (!displayField)
        return;
    displayField.value = isValEmpty(value) ? value : NLNumberToString(value);
}
...
function disableField(fld, val)
{
    if (!NS.UI.Util.isBackend)
    {
        ...
        else
        {
            ...
            if(datelink != null)
                datelink.style.visibility = val ? "hidden" : (NS.UI.Util.isRedwood ? "" : "inherit");
        }

		var span = fld.closest('.uir-field-input');
		if (span) {
			span.classList.toggle('uir-disabled', val);
		}
		if (NS.NextBridge) {
			NS.NextBridge.notify("nsUiButtonDisabled", { name: fld.name, value: val});
		}
    }
...
function nlExtOpenDivWindow(winname, width, height, fld, scrollbars, winTitle, listeners, html, triggerObj, chartHTML, winProperties) {
	...
	content.className = 'uir-window-frame';
    content.id = winname + '_frame';

    if (divhtml instanceof HTMLElement) {
	...
	var resizable = typeof winProperties.resizable !== 'undefined' ? winProperties.resizable : true;
	var closable = typeof winProperties.closable !== 'undefined' ? winProperties.closable : true;
...
function getObjectLeft(obj)
{
    ...
    return offset + (NS.UI.Util.isIE ? window.screenLeft : window.screenX + window.outerWidth - window.innerWidth);
}
...
function setFieldVisibility ( spanId, on )
{
	...
	if(NS.UI.Util.isNetsuiteNext){
		// In NetSuite Next, field comes together with label inside, therefore hide the whole field
		if (spanInput) {
			visible(spanInput.parentElement.parentElement, on);
		}
	} else {
    ...
    if (!NS.UI.Util.isBackend) {
		...
		if(NS.UI.Util.isNetsuiteNext){
			if(spanLabel == null){
				...
				var label = spanLabel.querySelector('.uir-label-text');
    ...
    * Note: this is distinct from ShowHideTab(), which activates/deactivates a tab.  This hides or replaces tabs. */
    function hideTab( tabName, hide )
    {
		...
		if (!NS.UI.Util.isBackend)
        {
			...
			if (currentlyDisplayed !== hide && !hide)
                return;
        }
	...
	/**
	 * Find all tabs on the same level
	 * @param tabName - name of the tab
	 * @param includeCurrent - include the tab with given tabName
	 * @returns {Array}
	 */
	function getTabSiblings(tabName, includeCurrent)
	{
		var siblings = [];
		var tab = getLayeredTab(tabName);
		if (tab)
		{
			var allSiblings = tab.parentNode.children;

			for (var i = 0; i < allSiblings.length; i++)
			{
				var elem = allSiblings[i];
				if (elem && elem.id.substring(elem.id.length-3) === 'lnk')
				{
					if (!!includeCurrent || elem !== tab)
					{
						siblings.push(elem);
					}
				}
	...
	/**
	 * Return true in case that at least one sibling tab is visible
	 * @param tabName
	 * @returns {boolean}
	 */
	function hasTabVisibleSibling(tabName)
	{
		var siblings = getTabSiblings(tabName, false);
		for (var i=0; i<siblings.length; i++)
		{
			if (siblings[i].style.display.indexOf('none') === -1)
			{
				return true;
			}
		}
	...
	function getLayeredTab(tabName)
	{
		return document.getElementById(tabName + 'lnk');
	}
    ...
    return {
        ...
        hideTab: hideTab,
	    hasTabVisibleSibling: hasTabVisibleSibling,
	    getTabSiblings: getTabSiblings,
	    getLayeredTab: getLayeredTab
    }
})();
...
function setFormValue(fld,value,text,firefieldchanged,synchronous)
{
	...
	if (fld.type == 'checkbox')
    {
		...
		if (text != null && text.length > 0 && fld.form && fld.form.elements) {
			var dispfld = fld.form.elements[fld.name + "_display"];
			if (dispfld) {
				dispfld.value = text;
			}
		}
...
function getFormElement(frm,fldname, fieldType)
{
    ...
    if ( frm != null )
    {
        if ( fldname == 'language' || (!NS.UI.Util.isBackend && (fldname == 'item' || fldname == 'cash')) )
        {
...
{
    var curleft = 0;
    var isNsLoaded = (window.parentAccesible && parent && parent.NS); //adding check for webstore Issue #203781
    var isWindowContainedInDivFrame = (isNsLoaded ? (!!parent.NS.Window.getActive()) : false);
    ...
    if (isWindowContainedInDivFrame) {
...
// set the text value to the field
...
/**
 * Cancels any further event capturing or bubbling. Use this method only when property <code>bubble</code> is set to
 * <code>true</code>.
...
function restoreHtmlEditors( frm )
{
    if ( typeof NetSuite == "object" && typeof NetSuite.RTEManager == "object" )
    {
		...
		try
		{
            ...
            NetSuite.RTEManager.forEach(function (editor) {
...
function getFlhUrl(fieldId, parentId, listId, variantId,
                   flhType, helpTopic, company, parentType, isTransactionForm,
                   formTitleTranKey, formTitle, fieldLabelTranKey, fieldLabel) {
	var params = {};
	var fp = NS.Help.flhUrlParams;
	params[fp.PARENT_ID.name] = parentId; //mandatory for STD FLH
	params[fp.LIST_ID.name] = listId;
	params[fp.FLH_TYPE.name] = flhType;
	params[fp.TOPIC_ALIAS_KEY.name] = helpTopic;
	params[fp.COMPANY.name] = company;
	params[fp.PARENT_TYPE.name] = parentType;
	params[fp.VARIANT_ID.name] = variantId;
	params[fp.TRANSACTION_FORM.name] = isTransactionForm;
	params[fp.FORM_TITLE_TRAN_KEY.name] = formTitleTranKey;
	params[fp.FORM_TITLE.name] = formTitle;
	params[fp.FIELD_LABEL_TRAN_KEY.name] = fieldLabelTranKey;
	params[fp.FIELD_LABEL.name] = fieldLabel;
	...
	return NS.Help.getFieldHelpUrl(options);
}

function nlFieldHelp(windowTitle, fieldId, fld,
                     parentId, listId, variantId, flhType, helpTopic,
                     company, parentType, isTransactionForm,
                     formTitleTranKey, formTitle, fieldLabelTranKey, fieldLabel)
{
	var url = getFlhUrl(fieldId, parentId, listId, variantId,
		flhType, helpTopic, company, parentType, isTransactionForm,
		formTitleTranKey, formTitle, fieldLabelTranKey, fieldLabel);

    if (NS && NS.Dashboard)
    {
        nlExtOpenWindow(url, 'fieldhelp', 400, 205, fld, false, windowTitle, {}, fld);
        return false;
    }
    ...
    if (typeof win == "undefined" ||  win == null || win.closed )
        win = window.open(url, winname, 'scrollbars='+(NS.UI.Util.isIE ? 'no' : 'yes')+',width='+Math.min(screen.availWidth,width)+',height=100,left=' + left + ',top=' + top + ',resizable=yes');
    else
    {
        win.location = url;
        win.moveTo(left, top);
...
function isLeftButtonDown(evnt)
{
    ...
    if (NS.UI.Util.isIE && e.button==1 || (!NS.UI.Util.isIE && e.button==0)) {
        bLeftClick = true;
    }
...
function eval_js(sScript)
{
    ...
    try {
        ...
        if(NS.UI.Util.isDebug || NS.Logger.debugValue) {
...
function adjustMachineData(fields, data, fieldsFromServer)
{
	if (!data || !fields)
		return data;
	var fieldNames = fields.split('\x01');
	if (fieldNames.length <= data.split('\x02', 1)[0].split('\x01').length)
		return data; // 1st line ok -> bail
	var lines = data.split('\x02');
	var fieldNamesFromServer = !fieldsFromServer ? fieldNames : fieldsFromServer.split('\x01');
	var needToCheckFieldsFromServer = !!fieldsFromServer && fields !== fieldsFromServer;
	lines = lines.map(function(line) {
		var i = 0;
		var values = line.split('\x01');
		var adjusted = fieldNames.map(function(fld) {
			return fld.substring(0, 8) === 'custpage' || (needToCheckFieldsFromServer && fieldNamesFromServer.indexOf(fld) < 0) ? '' : values[i++];
		});
		return i === values.length ? adjusted.join('\x01') : line;
	});
	return lines.join('\x02');
}
...
NLXMLHttpRequest.prototype.requestURL = function( url, postdata, requestHeaders, async, httpMethod )
{
	...
	if ( async )
	{
		this.xmlrequest.onreadystatechange = this.handleResponse.bind( this );
...
NLXMLHttpRequest.prototype.handleResponse = function()
{
	...
	if ( this.xmlrequest.readyState == 4 )  /* For now, we'll not worry about the Loading, Loaded, Interactive readyStates  */
	{
        ...
        /*
            kevinng 4/7/2011
            responseCode must be 200 or 206 or 500 with non-empty responseBody (see NLErrorPage.prepResponse())
...
NLJsonRpcClient.prototype =
{
	sendRequest : function (methodName, args, callback, httpMethod)
	{
		...
		var request = new NLXMLHttpRequest();
		if (callback != null)
			request.setResponseHandler(this.handleResponseAsync.bind(this));
...
function NLStringToNumber(str, addZeroPadding)
{
    ...
    if (addZeroPadding && number.toString().search(/e/i) < 0)
    {
        var paddingLength = 0;
...
function expandOrCollapseRows(machine, rownum, shouldCollapse)
{
    ...
    var EXPAND_ICON_URL = isRedwood ? "/uiredwood/icon/plus-square.svg" : "/images/forms/plus.svg";
    var COLLAPSE_ICON_URL = isRedwood ? "/uiredwood/icon/minus-square.svg" : "/images/forms/minus.svg";
...
function setLabel( spanId, label)
{
    if (!NS.UI.Util.isBackend)
    {
		...
		if(!NS.UI.Util.isNetsuiteNext){
			...
			var labelTextElement = spanLabel.querySelector(".uir-label-text");
			if (labelTextElement) {
...
{
	return (getEvent(evnt)) ? getEvent(evnt).keyCode : null;
}

function getEventMacCommandKey(evnt)
{
	return NS.UI.Util.isMac ? evnt.metaKey : false;
}

// checknotempty(): validate that field is not empty and return focus to offending field.
function checknotempty(fld1,nam)
{
	if (!checkvalnotempty(fld1.value, NS.Translations ? NS.Translations.getTranslatedString('NLAlertContext.PLEASE_ENTER_A_VALUE_FOR_1' , { 1: nam }) : parent.NS.Translations.getTranslatedString('NLAlertContext.PLEASE_ENTER_A_VALUE_FOR_1' , { 1: nam })))
	{
		try {
			fld1.focus();
			fld1.select();
...
function checkIsNotNegativeTime(field)
{
	...
	if (field.value != null && field.value.match(/^\s*-/)) {
		fireAlertWithTranslation('NLAlertContext.INVALID_NUMBER_MUST_BE_NON_NEGATIVE');
		valid = false;
	}
...
function checkzipcode(f, bIsCanada)
{
	...
	{
		if (bIsCanada)
			fireAlertWithTranslation('NLAlertContext.PLEASE_ENTER_A_VALID_1_DIGIT_POSTAL_CODE', { 1: n });
		else
			fireAlertWithTranslation('NLAlertContext.PLEASE_ENTER_A_VALID_1_DIGIT_ZIP_CODE', { 1: n });
		return false;
	}
	return true;
}
...
function extract_date_time(str)
{
	...
	if (spaceIdx > 0) {
		...
		return {
			validflag: false,
			alertMessageKey: 'NLAlertContext.INVALID_DATETIME_VALUE_MISS_SPACES_BETWEEN_DATE_AND_TIME'
		}
	}
}
...
function onlyUSPhoneFormat(field, type, doalert, autoplace, validflag) // former code from 'if' extracted to this function
{
	...
	if (val.length != 0 && val.length < 7) {
		if (doalert) fireAlertWithTranslation('NLAlertContext.PHONE_NUMBER_SHOULD_HAVE_SEVEN_DIGITS_OR_MORE');
		validflag = false;
	}
	...
	if (validflag && type == "fullphone") {
		...
		if (val.length != 0 && val.length < 10) {
			if (doalert) fireAlertWithTranslation('NLAlertContext.PLEASE_INCLUDE_THE_AREA_CODE_FOR_PHONE_NUMBER', null, val);
			validflag = false;
		}
	}
...
function NLDate_parseString(sDate, bDoAlert, id)
{
	...
	if(!window.dateformat)
		window.dateformat = "fmMM/DDfm/YYYY"; // Compatibility with calendar.jsp
	...
	else if(dateformat == "MM/DD/YYYY")
	{
		if (val.indexOf("/") != -1)
		{
			...
			// for payroll reports, we could have a format like MM/YYYY	hence, the date and year need to be set differently in that case
			if ( d > 1970 )
			{
				year = y = d;
				d = 1;
	...
	if(m==0 || d==0)
	{
		if(bDoAlert)
		{
			...
			fireAlertWithTranslation('NLAlertContext.INVALID_DATE_VALUE_MUST_BE_1', { 1: window.userfacing_dateformat });
		}
	}
	else
	{
...
function validatePeriodRange(fldPeriodStart, fldPeriodEnd)
{
	...
	{
		fireAlertWithTranslation('NLAlertContext.PLEASE_ENTER_A_VALID_DATE_RANGE_THE_FROM_DATE_MUST_PRECEDE_THE_TO_DATE');
		return false;
	}
...
{
	if (isNaN(dDate) || dDate.getTime() < NS.UI.Util.dateCutOff)
	{
		dDate = null;
		if (bDoAlert) {
			fireAlertWithTranslation('NLAlertContext.INVALID_DATE_VALUE_MUST_BE_ON_OR_AFTER_1CUTOFF_DATE', { 1: NS.UI.Util.dateCutOff });
		}
	}
	return dDate;
}
...
function validate_time(fldvalue, doalert, includeSeconds)
{
	...
	{
		fireAlertWithTranslation('NLAlertContext.INVALID_TIME_VALUE');
	}
...
function old_validate_field(field, type, doalert, autoplace, minval, maxval, mandatory, separator, scale)
{
	...
	if (field.value == null || field.value.length == 0)
	{
		...
		{
			if (doalert) {
				alertPromise = fireAlertWithTranslation('NLAlertContext.FIELD_MUST_CONTAIN_A_VALUE')
			}
	...
	if (type != "text" && type != "identifier" && type != "identifieranycase" && type != "address" && type != "visiblepassword" && NS.UI.Util.asianLang) {
	...
	if (type =="url")
	{
		...
		if (!(val.indexOf('/') == 0 || val.indexOf('http://') == 0 || val.indexOf('https://') == 0 || val.indexOf('ftp://') == 0 || val.indexOf('file://') == 0))
		{
			...
			{
				if (doalert)
					fireAlertWithTranslation('NLAlertContext.INVALID_URL_URL_MUST_START_WITH_HTTP_HTTPS_FTP_OR_FILE');
				validflag = false;
			}
			else
			{
		...
		{
			if (doalert)
				fireAlertWithTranslation('NLAlertContext.INVALID_URL_SPACES_ARE_NOT_ALLOWED_IN_THE_URL');
			validflag = false;
		}
	}
	else if (type == "currency" || type == "currency2" || type == "poscurrency" || type == 'nonnegcurrency')
	{
		...
		{
			/* The most common error case here is someone entered 'n/a', which triggers an expression evaluation.
			When the first char is not a digit, a javascript error will result.  We weed out these error cases by looking
			for if the string starts with an alpha character, and avoid the eval(). Normalize the string for the group
			separator and decimal separator, so that  the string is ready to be eval'ed. */
			...
			{
				/* Try/catch is only supported in relatively modern (post 4.x) browsers. The user might have an invalid expression
				that we are going to try if we run eval on an invalid expression it will throw a nasty error
		...
		{
			if (doalert){
				alertPromise = fireAlertWithTranslation('NLAlertContext.INVALID_CURRENCY_VALUE_VALUES_MUST_BE_NUMBERS_UP_TO_1', { 1: '999,999,999,999,999.99' });
			}
			validflag = false;
		}
		else if ( maxval != null && !isNaN(maxval) && Math.abs(numval)>=maxval )
		{
			if (doalert)
			{
				...
				alertPromise = fireAlertWithTranslation('NLAlertContext.INVALID_CURRENCY_VALUE_VALUES_MUST_BE_NUMBERS_UP_TO_1', { 1: preDecimal + '.999999999999999' });
			}

			validflag = false;
		}
		// doesn't handle min/maxval for currency in general - didn't want to add that now, too much chance for injection.
		// Only added handling min value of 0. EG 1/11/2006
		if ((type == "poscurrency" || type == "nonnegcurrency" || minval == 0) && numval < 0)
		{
			if (doalert) {
				alertPromise = fireAlertWithTranslation('NLAlertContext.INVALID_CURRENCY_VALUE_VALUE_CAN_NOT_BE_NEGATIVE');
			}
			validflag = false;
		}
		...
		if (validflag)
		{
			// If we are using "auto-place decimal" feature, then all prices that are input are treated as pennies, which we must divide by 100 before proceeding.
	...
	else if (type == "mmyydate")
	{
		...
		else
		{
			...
			if (doalert) fireAlertWithTranslation('NLAlertContext.INVALID_DATE_VALUE_MUST_BE_1', { 1: fmterr });
			validflag = false;
		}
	}
	else if (type == "ccexpdate" || type == "ccvalidfrom")
	{
		// This is a special date format in the form MM/YYYY for use specifically in VeriSign credit card transactions.
		validflag = true;
		var m=0, y=0;
		if(field.value.indexOf('/') != -1)
		{
			...
			// expiration date must be after today's date valid from / start date must be before today's date
			if(type == "ccexpdate" && (y < Y || (y==Y && m < M)) ||
				type == "ccvalidfrom" && (y > Y || (y==Y && m > M)))
			{
				if (doalert) fireAlertWithTranslation('NLAlertContext.NOTICE_THE_CREDIT_CARD_APPEARS_TO_BE_INCORRECT');
			}
		...
		else
		{
			if (doalert)
			{
				if (type == "ccexpdate") fireAlertWithTranslation('NLAlertContext.PLEASE_ENTER_AN_EXPIRATION_DATE_IN_MMYYYY_FORMAT');
				else fireAlertWithTranslation('NLAlertContext.PLEASE_ENTER_A_VALID_FROM_START_DATE_IN_MMYYYY_FORMAT');
			}
			validflag = false;
		}
	}
	...
	else if (type == "rate" || type == 'ratehighprecision')
	{
		...
		if (isNaN(numval))
		{
			if (doalert) fireAlertWithTranslation('NLAlertContext.INVALID_NUMBER_OR_PERCENTAGE');
			validflag = false;
		}
		else
		{
	...
	else if (type == "integer" || type == "posinteger" || type == "float" || type == "posfloat" || type == "nonnegfloat" || type == "percent" || type == "dynamicprecision")
	{
		...
		if (isNaN(numval) || (custrange && (numval > maxclip || numval < minclip)) || (!custrange && (numval >= maxclip || numval <= minclip)))
		{
			if (doalert)
			{
				if (type == "percent")
				{
					fireAlertWithTranslation('NLAlertContext.INVALID_PERCENTAGE_MUST_BE_BETWEEN_1_AND_2', { 1: minclip, 2: maxclip });
				}
				else if (custrange == true)
				{
					if (minval == null)
						fireAlertWithTranslation('NLAlertContext.INVALID_NUMBER_MUST_BE_AT_LEAST_1', { 1: maxclip });
					else if (maxval == null)
						fireAlertWithTranslation('NLAlertContext.INVALID_NUMBER_MUST_BE_AT_LEAST_1', { 1: minclip });
					else
						fireAlertWithTranslation('NLAlertContext.INVALID_NUMBER_MUST_BE_BETWEEN_1_AND_2', { 1: minclip, 2: maxclip });
				}
				else if (type=="posinteger" || type=="posfloat")
					fireAlertWithTranslation('NLAlertContext.INVALID_NUMBER_MUST_BE_POSITIVE');
				...
				{
					if (isNaN(numval))
						fireAlertWithTranslation('NLAlertContext.YOU_MAY_ONLY_ENTER_NUMBERS_INTO_THIS_FIELD');
					else
						fireAlertWithTranslation('NLAlertContext.ILLEGAL_NUMBER', null, numval);
				}
				else
				fireAlertWithTranslation('NLAlertContext.INVALID_NUMBER_MUST_BE_GREATER_THAN_429B');
			}
			validflag = false;
		}
		else
	...
	else if (type == "address")
	{
		...
		{
			err = NS.Translations ? NS.Translations.getTranslatedString('NLAlertContext.ADDRESS_TOO_LONG_TRUNCATED_AT_1000_CHARACTERS') :  parent.NS.Translations.getTranslatedString('NLAlertContext.ADDRESS_TOO_LONG_TRUNCATED_AT_1000_CHARACTERS');
	...
	else if (type == "function")
	{
		...
		if (!re.test(val))
		{
			if  (doalert) fireAlertWithTranslation('NLAlertContext.THE_FUNCTION_FIELD_MUST_BE_A_VALID_JAVASCRIPT_FUNCTION_IDENTIFIER');
			validflag = false;
		}
	}
	else if (type == "time" || type == "timetrack")
	{
		...
		var re = (type === "timetrack") ? /([0-9]+)?(:[0-9][0-9]+)?/ : /([0-9][0-9]?)?(:[0-9][0-9]+)?/;
		...
		// if the user entered a value >= 60 for minutes, add the extra hours to the hours var and reduce minutes to be less than 60 (issue 48406)
		if (minutes >= 60)
		{
		...
		{
			if (doalert) fireAlertWithTranslation('NLAlertContext.INVALID_TIME_VALUE_MUST_BE_HHMI');
			validflag = false;
		}
	}
	...
	else if (type == 'datetimetz' || type == 'datetime')
	{
		...
		if (validflag)
		{
			...
			alertPromise = fireAlertWithTranslation(ret_date_time.alertMessageKey);
		}
	}
	else if (type == "email")
	{
	...
	else if (type == "emails")
	{
		...
		{
			validflag = false;
			if (doalert) fireAlertWithTranslation('NLAlertContext.INVALID_EMAILS_FOUND', null , bademails.join('; '));
		}
		else if (validcount < 1)
		{
			validflag = false;
			if (doalert) fireAlertWithTranslation('NLAlertContext.NO_VALID_EMAILS_FOUND_IN_1', { 1: field.value });
		}
	}
	else if (type == "phone" || type == "fullphone")
	{
		if (NS.UI.Util.internationalPhoneNumbers) {
			if (typeof require === 'function') {
				require(['N/format/i18n', 'N/runtime'], function (formatI18n, runtime) {
					var fmtNumber = formatI18n._formatPhoneUIField(field.value, formatI18n.SubsidiaryCountry, true, true, formatI18n.PhoneFieldMaxLength);
					field.value = fmtNumber;
				});
			} else {
				validflag = onlyUSPhoneFormat(field, type, doalert, autoplace, validflag); // fallback solution
			}
		} else {
	...
	else if (type == "color")
	{
		...
		{
			if (doalert) fireAlertWithTranslation('NLAlertContext.COLOR_VALUE_MUST_BE_6_HEXADECIMAL_DIGITS_OF_THE_FORM_RRGGBB__EXAMPLE_FF0000_FOR_RED');
			validflag = false;
		}
		else
			field.value = "#"+val;
	...
	else if (type == "identifier" || type == "identifieranycase")
	{
		...
		if (!re.test(val))
		{
			if (doalert) fireAlertWithTranslation('NLAlertContext.IDENTIFIERS_CAN_CONTAIN_ONLY_DIGITS_ALPHABETIC_CHARACTERS_OR__WITH_NO_SPACES');
			validflag = false;
		}
		else
	...
	else if (type == "package")
	{
		...
		if (!validflag && doalert)
			fireAlertWithTranslation('NLAlertContext.INVALID_SUITEAPP_APPLICATION_ID');
		else
	...
	else if (type == "furigana")
	{
		...
		if (!re.test(val))
		{
			if (doalert) fireAlertWithTranslation('NLAlertContext.NON_KATAKANA_DATA_FOUND');
			validflag = false;
		}
	}
	else if(type == "urlcomponent")
	{
		...
		var re = /^[a-z0-9\-]*$/;
		if(!re.test(val)) {
			if(doalert) fireAlertWithTranslation('NLAlertContext.THIS_FIELD_COULD_CONTAIN_ONLY_LOWER_CASE_LETTERS_NUMBERS_AND_');
			validflag = false;
		}
		else
			field.value = val;
	...
	if (mandatory == true)
	{
		...
		{
			if (doalert) fireAlertWithTranslation('NlAlertContext.FIELD_MUST_CONTAIN_A_VALUE');
			validflag = false;
		}
	}
	if (!validflag && field.modalOpened !== true) {
		field.modalOpened = true;
		alertPromise.then(function(){
			selectAndFocusField(field);
			field.modalOpened = false;
		})
	}
...
function validate_textfield_maxlen(field, maxLen, bAlert, bMaxInChars)
{
	...
	if (bMaxInChars)
	{
		...
		{
			if (bAlert)
				fireAlertWithTranslation('NLAlertContext.YOU_HAVE_EXCEEDED_THE_1_CHARACTER_LIMIT_FOR_THIS_FIELD_PLEASE_SHORTEN_YOUR_ENTRY_BY_2_CHARACTERS', { 1: maxLen, 2: len-maxLen });
	...
	else
	{
		...
		{
			if (bAlert)
				fireAlertWithTranslation('NLAlertContext.YOU_HAVE_EXCEEDED_THE_LENGTH_LIMIT_FOR_THIS_FIELD_PLEASE_SHORTEN_YOUR_ENTRY_BY_1_CHARACTERS', { 1: toTrim });
...
if (!NS.UI.Util.isBackend)
{
	...
	if(!alert.isNextAlert){
		...
		window.alert.isNextAlert = alert_inner.isNextAlert;
	}
...
function expandCollapseFieldArrangement(id) {
	var toggleButton = document.getElementById('arrw_' + id);
	toggleButton.classList.toggle('uir-field-arrangement--collapse-button-collapsed');
	toggleButton.classList.toggle('uir-field-arrangement--collapse-button-expanded');

	var moreOptions = document.getElementById('tr_' + id);
	moreOptions.style.display = moreOptions.style.display==='none' ? '' : 'none';
}

function syncFieldValuesWithBFCache() {
	window.addEventListener('pageshow', function (event) {
		if (event.persisted || performance.getEntriesByType("navigation")[0].type === 'back_forward') {
			document.querySelectorAll('[data-fieldtype] > input[type=text],[data-fieldtype] > textarea').forEach(function (input) {
				if (input.value !== input.defaultValue && input.onchange) {
					input.onchange();
				}
			});
		}
...
// send the proper request to a frame
...
/**
 * Gets name and value of post field that is expected by server as CSRF token
 * @returns {{name:string, value: string}}
 */
NS.getCSRFToken = function() {
	try {
		return JSON.parse(document.getElementById('ns-csrf-token').innerText);
	} catch (e) {
		throw new Error("Cannot load CSRF token", { cause: e });
	}
}

/**
 * Sends data (provided by data parameter or extracted from url) using POST request to server and redirect page if needed
 * @param {string} url
 * @param {object} [data={}] key-value data
 * @param {string} [target='_self'] target frame
 */
NS.submitMutationAction = function submitMutationAction(url, data, target) {
  var form = document.createElement("form");
  form.method = "POST";
  form.target = target || "_self";
  form.style.display = "none";

  var iframeTarget = form.target != "_self";
  var parsedUrl = new URL(url, window.location.href);

  if (data) {
	if (iframeTarget) {
		parsedUrl.searchParams.append("nextSubIframeTarget", "T");
	}
    form.action = parsedUrl;
    for (var key in data) {
      addInput(form, key, data[key]);
    }
  } else {
    form.action = parsedUrl.pathname + (iframeTarget ? "?nextSubIframeTarget=T" : "");
    parsedUrl.searchParams.forEach(function (value, key) {
      addInput(form, key, value);
    });
  }

  var csrfToken = NS.getCSRFToken();
  addInput(form, csrfToken.name, csrfToken.value);
...
NS.UI.Util.downloadFile = function (url, options) {
	var parsedUrl = new URL(url, window.location.href);
	...
	if (options.method === 'GET') {
		...
		parsedUrl.searchParams.set("nextDownloadLink", "");
		...
		var form = document.createElement("form");
		form.method = "POST";
		form.action = parsedUrl.origin + parsedUrl.pathname + "?nextDownloadLink=";
		form.style.display = "none";
